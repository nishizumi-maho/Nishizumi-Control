"""
Dominant Control para iRacing
~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Um aplicativo abrangente de gerenciamento de controles para o iRacing que oferece:
- Ajuste dinâmico de controles do piloto (bias de freio, controle de tração etc.)
- Suporte a múltiplos dispositivos de entrada (teclado, joystick, botões do volante)
- HUD sobreposto com telemetria em tempo real
- Gerenciamento de presets por carro e por pista
- Sistema de macros/combos para ajustes rápidos

Autor: Nishizumi Maho
...
# Tempos profiles for input simulation (click-click timing)
GLOBAL_TIMING = {
...
def mark_pending_scan():
    ...
    try:
        ...
        print(f"[PendingVarredura] Failed to persist marker: {exc}")
...
def consume_pending_scan() -> bool:
    ...
    try:
        ...
        print(f"[PendingVarredura] Failed to clear marker: {exc}")
...
def press_key(scan_code: int):
    ...
    if SendInput is None:
        raise OSErro("SendInput APIs are only available on Windows platforms.")
...
def _normalize_timing_config(timing: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key in [
        ...
        except (TipoErro, ValueErro, KeyErro):
...
class InputManager:
        ...
        if self.safe_mode:
            ...
            if HAS_PYGAME:
                try:
                    ...
                    print(f"[InputManager] Erro reactivating pygame: {e}")
    ...
    def get_all_devices(self) -> List[Tuple[int, str]]:
        ...
        try:
            ...
            print(f"[InputManager] Erro getting devices: {e}")
            return []
...
class VoiceListener:
    ...
    def _init_vosk_model(self, model_path: str):
        """Carregar the Vosk model from disk if available."""
        ...
        try:
            with sr.Microphone(device_index=self.device_index) as source:
                ...
                while self.running:
                    ...
                    except getattr(sr, "WaitTimeoutErro", Exception):
                        continue
                    except Exception:
                        continue
...
class VoiceTestDialog(tk.Toplevel):
    ...
    def __init__(
        ...
        self.title("Teste de voz e macro")
        self.geometry("430x360")

        info = tk.Label(
            self,
            text=(
                "Fale uma das frases configuradas para acionar a macro.\n"
                "Use o botão de teste para garantir que seu microfone e suas"
                " frases estejam funcionando."
            ),
            wraplength=400,
            justify="left"
        )
...
class DeviceSelector(tk.Toplevel):
    """
        Diálogo para selecionar quais dispositivos USB o aplicativo pode usar.
    """
    ...
    def __init__(self, parent, current_allowed: List[str], callback: Callable[[List[str]], None]):
        ...
        tk.Label(
            self,
            text="Marque/desmarque para permitir ou bloquear o uso do dispositivo",
            fg="gray"
        ).pack()
...
class OverlayConfigTab(tk.Frame):
    ...
    def __init__(self, parent, app):
        ...
        tk.Label(
            self.body,
            text="Configuração do HUD / Sobreposição",
            font=("Arial", 11, "bold")
        ...
        feedback_frame = tk.LabelFrame(
            self.body, text="Limiares de feedback dos assistentes (por carro)"
        )
        feedback_frame.pack(fill="x", padx=5, pady=5)

        tk.Checkbutton(
            feedback_frame,
            text="Mostrar avisos de ABS / TC / derrapagem no HUD",
        ...
        tk.Label(header, text="Mostrar", width=8, anchor="w").pack(
            side="left", padx=2
        )
        tk.Label(header, text="Nome interno", width=25, anchor="w").pack(
            side="left", padx=2
        )
        tk.Label(header, text="Rótulo no HUD", width=20, anchor="w").pack(
            side="left", padx=2
        )
        ...
        tk.Label(
            self.body,
            text="Seleções e rótulos de variáveis são salvos por carro.\n"
    ...
    def load_for_car(
        ...
        """
        Carregar HUD configuration for a specific car.
    ...
    def _on_feedback_change(self, *_args):
        ...
        car = self.app.current_car or "Carro genérico"
    ...
    def _load_feedback_for_car(self, car_name: str) -> None:
        """Carregar per-car feedback thresholds into the UI fields."""
...
class GenericController:
    ...
    def find_minimum_effective_timing(
        ...
        """
        if not self.key_increase or not self.key_decrease:
            raise ValueErro("Increase/decrease keys must be configured before probing.")
...
class ControlTab(tk.Frame):
    ...
    def __init__(self, parent, controller: GenericController, label_name: str, app):
        ...
        tk.Label(
            presets_frame,
            text="Redefinir sempre retorna a um valor base (ex.: 0 ou 50). Adicione abaixo seus valores de macro preferidos.",
            fg="gray",
            font=("Arial", 8),
            wraplength=760,
            justify="left"
        ...
        tk.Label(
            ...
            text=(
                ...
                "As configurações de voz/áudio ficam em Opções → Voz/Áudio."
            ),
            fg="gray",
            font=("Arial", 8),
            wraplength=760,
        ...
        tk.Label(
            header,
            text="Frase de ativação por voz",
            width=20,
            anchor="w",
            font=("Arial", 8, "bold")
        ).pack(side="left", padx=5)
            ...
            if suggested is None:
                self.after(
                    ...
                    lambda: messagebox.showwarning(
                        ...
                        "Nenhum tempo entre 1-120 ms atualizou a telemetria de forma confiável."
                    )
                )
            else:
                msg = (
                ...
                self.after(0, lambda: messagebox.showinfo("Resultado da sondagem", msg))
    ...
    def add_preset_row(
        ...
        label_text = "Redefinir" if is_reset else "Macro"
        tk.Label(
            frame,
            text=label_text,
            width=6,
    ...
    def set_config(self, config: Dict[str, Any]):
        ...
        self.btn_decrease.config(text=config.get("key_decrease_text", "Definir redução (-)"))
...
# Due to length, I'll create a third artifact for ComboTab, GlobalTemposWindow, 
...
class ComboTab(tk.Frame):
    ...
    def __init__(
        ...
        tk.Label(
            body,
            text="⚡ Ajustes combinados (um gatilho -> múltiplas variáveis)",
            fg="orange",
            font=("Arial", 10, "bold")
        ).pack(pady=5)
    ...
    def add_dynamic_row(
        ...
        bind_button = tk.Button(
            frame,
            text="Redefinir" if is_reset else "Definir atalho",
            width=15,
            fg="red" if is_reset else "black"
        )
        ...
        # Excluir button (except for Redefinir)
        if not is_reset:
            tk.Button(
                frame,
                text="X",
        ...
        # Carregar existing data if provided
        if existing:
...
# ======================================================================
class GlobalTemposWindow(tk.Toplevel):
    """
    ...
    def __init__(self, parent, callback_save: Callable):
        super().__init__(parent)
        self.title("Ajustes de tempo (antidetecção)")
        ...
        notebook.add(timing_frame, text="Tempos")
        ...
        tk.Radiobutton(
            profiles_frame,
            text="🤖 BOT (experimental, quase sem atraso)",
            variable=self.var_profile,
            value="bot",
            command=self._on_profile_change
        ).pack(anchor="w", padx=5, pady=2)
        ...
        tk.Label(self.custom_frame, text="Pressionar mín. (ms):").grid(
        ...
        tk.Label(self.custom_frame, text="Pressionar máx. (ms):").grid(
...
class iRacingControlApp:
    ...
    def __init__(self, root: tk.Tk):
        ...
        # Mostrar overlay if it was visible
    ...
    def _create_menu(self):
        ...
        options_menu.add_command(
            label="Mostrar/ocultar sobreposição",
        ...
        options_menu.add_separator()
        options_menu.add_command(
            label="Restaurar padrões (Excluir Config)",
    ...
    def _create_main_ui(self):
        ...
        self.check_safe = tk.Checkbutton(
            settings_frame,
            text="Modo somente teclado (requer reinício)",
        ...
        tk.Label(
            settings_frame,
            text="(Sem botões de joystick/volante)",
            fg="gray",
            font=("Arial", 8)
        ).pack(side="left", padx=4)
        ...
        tk.Checkbutton(
            auto_frame,
            text="Detectar automaticamente carro/pista via iRacing",
            variable=self.auto_detect
        ).pack(anchor="w")

        tk.Checkbutton(
            auto_frame,
            text="Reescanear automaticamente quando carro/pista mudarem",
        ...
        stability_frame = tk.LabelFrame(
            self.root,
            text="Opções de estabilidade"
        )
        stability_frame.pack(fill="x", padx=10, pady=5)

        tk.Checkbutton(
            stability_frame,
            text="Reiniciar antes de reescanear controles (após a primeira varredura)",
        ...
        tk.Checkbutton(
            stability_frame,
            text="Reiniciar e escanear automaticamente ao entrar em sessão de corrida",
        ...
        tk.Checkbutton(
            stability_frame,
            text="Mostrar pop-up de conclusão da varredura",
        ...
        tk.Checkbutton(
            stability_frame,
            text="Continuar tentando alcançar as teclas de destino (sem tempo limite)",
        ...
        tk.Label(
            clear_frame,
            text="Limpar tecla de destino (opcional):"
        ).pack(side="left")
        self.btn_clear_target_bind = tk.Button(
            clear_frame,
            text="Definir tecla de limpeza",
            width=18,
        ...
        tk.Checkbutton(
            presets_frame,
            text="Salvar automaticamente alterações do preset (atalhos/macros)",
        ...
        devices_frame = tk.LabelFrame(
            self.root,
            text="Etapa 2: Confirmar dispositivos de entrada (joystick/volante)"
        )
        ...
        self.btn_scan = tk.Button(
            self.root,
            text="Etapa 3: Escanear controles do piloto para o carro selecionado",
        ...
        tk.Label(
            self.root,
            text="Dica: Faça a varredura após alterar dispositivos ou presets para manter os vínculos em sincronia.",
            fg="gray",
            font=("Arial", 9)
    ...
    def _build_voice_audio_tab(self, parent: tk.Widget):
        ...
        ttk.Label(engine_frame, text="Motor de voz:").pack(side="left", padx=4)
    ...
    def toggle_mode(self):
        ...
        if self.app_state == "RUNNING":
            ...
            self.btn_mode.config(text="Modo: EXECUTANDO", bg="#90ee90")
    ...
    def load_specific_preset(self, car: str, track: str):
        ...
        self.register_current_listeners()
        print(f"[Preset] Carregared {car} / {track}")

    def action_load_preset(self):
        """Carregar selected preset."""
        car = self.combo_car.get()
    ...
    def _auto_scan_and_load_preset(self) -> None:
        """Varredura controls and then reload the current car/track preset."""
        self._session_scan_pending = False
        self._skip_next_auto_load = False
    ...
    def scan_driver_controls(self):
        """Varredura for dc* driver control variables in current car."""
        ...
        try:
            ...
            print(f"[Varredura] Erro reading variables: {e}")

        if not found_vars:
            messagebox.showwarning(
                "Varredura",
                "Nenhuma variável numérica 'dc*' encontrada.\n"
                "O carro pode não ter controles de piloto ou você não está no modo Drive."
            )
            return
        ...
        if preset_data.get("tabs") or preset_data.get("combo"):
            # Carregar preset will rebuild tabs with configs and re-register listeners
            self.load_specific_preset(car, track)
        else:
    ...
    def rebuild_tabs(self, vars_list: List[Tuple[str, bool]]):
        ...
        self.notebook.add(combo_frame, text="⚡ Combinações")
        ...
        self.notebook.add(overlay_frame, text="HUD / Sobreposição")

        # Carregar overlay for current car
        car = self.current_car or "Carro genérico"
    ...
    def open_timing_window(self):
        ...
        GlobalTemposWindow(self.root, self.save_timing_config)
    ...
    def save_config(self):
        ...
        try:
            ...
            print(f"[SAVE] Erro saving config: {e}")

    def load_config(self):
        """Carregar configuration from disk."""
    ...
    def _format_vosk_status(self) -> str:
        ...
        if not model_path:
            return "Selecione a pasta do modelo Vosk"
    ...
    def _format_whisper_status(self) -> str:
        ...
        if not self.whisper_model_path.get():
            return "Selecione um modelo .bin/.gguf"
    ...
    def choose_whisper_binary(self):
        ...
        path = filedialog.askopenfilename(
            title="Selecionar executável do whisper.cpp",
    ...
    def restore_defaults(self):
        """Excluir the configuration file and restart the app after confirmation."""
        if not messagebox.askyesno(
            "Restaurar padrões",
        ...
        try:
            ...
            messagebox.showerror(
                "Erro",
                f"Falha ao excluir a configuração: {exc}"
            )
            return
