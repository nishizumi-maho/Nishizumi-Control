...
QUICKSTART_TEXT = (
    "1. Join the session and click Drive to start the telemetry.\n2. Run a few representative laps, clean ones if you can.\n3. Choose how many laps should build the first reference.\n4. Use the minimal view during the stint to take up less room.\n5. Pop the circle out to follow it in a window of its own."
)
...
class TractionCircleOverlay:
    def __init__(self, root: tk.Misc | None = None, ir_client: Any = None) -> None:
        self.ir = ir_client if ir_client is not None else irsdk.IRSDK()
        self.integrated = root is not None
        self.root = root if root is not None else tk.Tk()
        if not self.integrated:
            self.root.title("Nishizumi Traction")
        ...
        self.reference_var = tk.StringVar(value="Live adaptive reference")
        ...
        self.footer_var = tk.StringVar(
            value="M: compact/detailed  •  V: minimal view  •  O: pop the circle out  •  L: LapDist  •  S: settings"
        )
        self.settings_hint_var = tk.StringVar(value="Advice after 5 clean laps.")
        ...
        self._circle_items: Dict[tk.Canvas, Dict[str, Any]] = {}
        self.live_long_g = 0.0
        self.live_lat_g = 0.0
        self.live_usage_pct = 0.0
    ...
    def _build_style(self) -> None:
        ...
        # This view shares a process with the other tabs; only namespaced
        # styles may be changed here.
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(title_wrap, text="Nishizumi Traction", style="Title.TLabel").pack(anchor="w")
        ...
        self.btn_mode = ttk.Button(toolbar, text="Compact advice (M)", command=self._toggle_mode)
        self.btn_mode.pack(side="left", padx=(8, 0))
        self.btn_circle = ttk.Button(toolbar, text="Pop the circle out (O)", command=self._toggle_circle_popout)
        self.btn_circle.pack(side="left", padx=(8, 0))
        self.btn_lapdist = ttk.Button(toolbar, text="Overlay LapDist (L)", command=self._toggle_lapdist_overlay)
        self.btn_lapdist.pack(side="left", padx=(8, 0))
        self.btn_sidebar = ttk.Button(toolbar, text="Settings panel (S)", command=self._toggle_sidebar)
        ...
        ttk.Button(toolbar, text="Use the live reference", command=self._clear_ibt_reference).pack(side="right", padx=(0, 8))
        ...
        ttk.Label(self.sidebar, text="Advice", style="Section.TLabel").pack(anchor="w")
        self.coach_intro = ttk.Label(
            self.sidebar,
            text="The biggest grip losses come first.",
            style="Hint.TLabel",
            wraplength=340,
            justify="left",
        )
        ...
        ttk.Label(self.sidebar, text="Settings", style="Section.TLabel").pack(anchor="w")

        self.settings_box = ttk.Frame(self.sidebar, style="Card.TFrame", padding=14)
        self.settings_box.pack(fill="x", pady=(10, 0))
        ttk.Label(self.settings_box, text="Laps to learn from", style="Body.TLabel").grid(row=0, column=0, sticky="w")
        ...
        ttk.Checkbutton(
            self.settings_box,
            text="Learn from clean laps only",
        ...
        ttk.Label(
            self.settings_box,
            text="Opens in a window of its own so it is easier to read.",
            style="Hint.TLabel",
            wraplength=180,
            justify="left",
    ...
    def _on_drag_motion(self, event: tk.Event[tk.Misc]) -> None:
        if self.integrated or not self.minimal_mode or self._drag_start is None:
            return
    ...
    def _open_circle_popout(self) -> None:
        ...
        ttk.Label(shell, text="Traction circle popped out", style="Section.TLabel").pack(anchor="w")
        ttk.Label(
            shell,
            text="Follow how much grip you are using while you drive.",
            style="Hint.TLabel",
            wraplength=260,
            justify="left",
        ...
        self.btn_circle.configure(text="Put the circle back (O)")
    ...
    def _close_circle_popout(self) -> None:
        ...
        if self.circle_canvas is not None:
            self._circle_items.pop(self.circle_canvas, None)
        self.circle_canvas = None
        self.circle_caption_var.set("Usage --")
        self.btn_circle.configure(text="Pop the circle out (O)")
    ...
    def _close_lapdist_overlay(self) -> None:
        ...
        self.btn_lapdist.configure(text="Overlay LapDist (L)")
    ...
    def _get_nested(data: object, *path: object) -> object:
        ...
        for item in path:
            if isinstance(item, int):
                if not isinstance(cur, (list, tuple)) or item < 0 or item >= len(cur):
                    return None
                cur = cur[item]
                continue
    ...
    def _is_offtrack(self, driver_car_idx: int) -> bool:
        ...
        if not isinstance(surfaces, (list, tuple)) or driver_car_idx < 0 or driver_car_idx >= len(surfaces):
            return False
    ...
    def _reference_from_ibt(self, file_path: str) -> Optional[List[float]]:
        ...
        if ibt_reader is None:
            ...
            self.subheadline_var.set("This version of pyirsdk cannot read IBT files.")
            return None
        ...
        try:
            ...
            self.subheadline_var.set(f'Could not open the IBT: {exc}')
            return None

        if opened is False:
            self.status_var.set("IBT error")
            self.subheadline_var.set("Could not open the IBT file.")
            return None
        ...
        if not lap_dist or not long_accel or not lat_accel:
            self.status_var.set("IBT incompleto")
            self.subheadline_var.set("The IBT has no LapDistPct, LongAccel and LatAccel.")
            return None
        ...
        if max(bins, default=0.0) < MIN_REFERENCE_G:
            self.status_var.set("IBT with no reference")
            self.subheadline_var.set("The IBT did not produce a useful grip reference.")
            return None
        return bins
    ...
    def _load_ibt_reference(self) -> None:
        ...
        self._load_ibt_reference_path(file_path)

    def _load_ibt_reference_path(self, file_path: str) -> bool:
        """Load a selected IBT path, including requests from an external view."""
        ...
        lap_label = "clean laps" if self.incident_free_only_var.get() else "laps completed"
        self.subheadline_var.set(f'IBT reference loaded. Advice after {lap_target} {lap_label}.')
        return True

    def _clear_ibt_reference(self) -> None:
        self.external_reference_bins = None
        self.external_reference_path = None
        self.reference_var.set("Live adaptive reference")
        lap_label = "clean laps" if self.incident_free_only_var.get() else "laps completed"
        self.subheadline_var.set(f'Using an adaptive reference built from your {lap_label}.')

    @staticmethod
    def _phase_and_recommendation(neg_long: float, lat: float, pos_long: float) -> Tuple[str, str]:
        if neg_long > max(lat, pos_long):
            return "Entry", "Try braking a little later, or releasing the brake peak more smoothly."
        if lat >= max(neg_long, pos_long):
            return "Mid-corner", "There is room for more minimum speed; release the brake a touch earlier and aim for the apex."
        return "Exit", "There is margin on exit; get on the throttle a touch earlier and feed it in."
    ...
    def _refresh_feedback_settings(self, _event: object = None) -> None:
        lap_target = self._feedback_lap_target()
        lap_label = "clean laps" if self.incident_free_only_var.get() else "laps completed"
        self.settings_hint_var.set(f'Advice after {lap_target} {lap_label}.')
    ...
    def _open_quickstart_window(self, _event: object = None) -> None:
        ...
        ttk.Label(card, text="Quick Guide", style="Section.TLabel").pack(anchor="w")
        ttk.Label(
            card,
    ...
    def _toggle_mode(self, _event: object = None) -> None:
        ...
        self.btn_mode.configure(text=("Compact advice (M)" if self.compact_mode else "Detailed advice (M)"))
    ...
    def _apply_layout_mode(self) -> None:
        if self.minimal_mode:
            ...
            self.btn_minimal.configure(text="Leave the minimal view (V)")
            self.btn_circle.configure(text=("Put the circle back (O)" if self.circle_window is not None and self.circle_window.winfo_exists() else "Pop the circle out (O)"))
            self.btn_lapdist.configure(
                text=("Hide LapDist (L)" if self.lapdist_window is not None and self.lapdist_window.winfo_exists() else "Overlay LapDist (L)")
            )
            self.footer_var.set("Drag to position it  •  M: compact/detailed  •  V: exit")
        else:
            ...
            self.footer_var.set("M: compact/detailed  •  V: minimal view  •  O: pop the circle out  •  L: LapDist  •  S: settings")

    def _format_compact_headline(self, segments: Sequence[UnderuseSegment], lap_label: str) -> Tuple[str, str]:
        if not segments:
            return (
                "No strong advice yet",
                f'Keep driving to widen the reference with another {lap_label}.',
            )
        top = segments[0]
        headline = f'Biggest difference: Δ{top.delta_g:.2f}g in {top.phase.lower()}'
    ...
    def _update_coach_cards(self, segments: Sequence[UnderuseSegment], lap_label: str, coaching_ready: bool) -> None:
        if not coaching_ready:
            waiting = f'Waiting for {lap_label} before the coaching starts.'
            meta = "Building your reference"
        ...
        if not segments:
            cards[0].set("No clear difference", "Good consistency", "The last laps showed no meaningful grip loss.")
            cards[1].set("", "", "")
            cards[2].set("", "", "")
            return
        ...
        for i, card in enumerate(cards):
            if i >= len(segments):
                ...
                continue
            seg = segments[i]
            priority = "high" if seg.severity == "high" else "average" if seg.severity == "medium" else "low"
            confidence = "good confidence" if seg.confidence else "low confidence"
            trend = {
                "improving": "improving",
                "declining": "declining",
                "stable": "steady",
            }.get(seg.trend, seg.trend)
            title = f"#{i+1}  {seg.phase}  •  Δ{seg.delta_g:.2f}g"
            meta = f'Priority {priority}  •  {trend}  •  {seg.consistency:.0f}% of the laps  •  {confidence}'
    ...
    def _render_circle(self, canvas: tk.Canvas, long_g: float, lat_g: float, usage_pct: float, *, compact: bool) -> None:
        """Draw the traction circle without flickering.

        Deleting and recreating every item each tick (the previous approach)
        forces Tk to briefly paint an empty canvas before the new items land,
        which is the visible blink. Static items (ring, grid, labels, gauge
        background) are now created once and cached per canvas; only the
        parts that actually move (accel dot/line, usage text, gauge fill) get
        updated in place via ``coords``/``itemconfigure`` on every later
        tick. A full rebuild only happens on the first draw for a canvas, or
        when its size actually changes (e.g. the window was resized).
        """
        ...
        items = self._circle_items.get(canvas)
        ...
        if items is None or items.get("layout_key") != layout_key:
            ...
            self._circle_items[canvas] = items
    ...
    def _build_circle_items(
        ...
        radius: float,
        compact: bool,
        layout_key: Tuple[int, int, bool],
    ) -> Dict[str, Any]:
        """Create every circle canvas item once for the given layout."""
        canvas.delete("all")
        items: Dict[str, Any] = {"layout_key": layout_key}
        ...
        items["accel_line"] = canvas.create_line(cx, cy, cx, cy, fill=GOOD, width=3)
        items["dot"] = canvas.create_oval(cx, cy, cx, cy, fill=DOT, outline="")
        ...
        if compact:
            ...
            items["usage_text"] = canvas.create_text(
                cx, h - 22, text="", fill=GOOD, font=("Segoe UI Semibold", 10)
            )
            return items
        ...
        items["usage_text"] = canvas.create_text(
            cx, label_y, text="", fill=GOOD, font=("Segoe UI Semibold", 11)
        )
        ...
        items["gauge_fill"] = canvas.create_rectangle(gx0, gy0, gx0, gy1, fill=GOOD, outline="")
        items["gauge_geo"] = (gx0, gy0, gy1, gauge_w)
        return items

    def _draw_circle(self, long_g: float, lat_g: float, usage_pct: float) -> None:
        self.live_long_g = float(long_g)
        self.live_lat_g = float(lat_g)
        self.live_usage_pct = float(usage_pct)
    ...
    def _update_disconnected_ui(self) -> None:
        ...
        self.coach_card_1.set("Not connected", "iRacing not detected", "Learning picks up again on its own once telemetry arrives.")
        self.coach_card_2.set("", "", "")
        self.coach_card_3.set("", "", "")
        self.lapdist_var.set("LapDist: --\nWaiting for iRacing...")
        self._draw_circle(0.0, 0.0, 0.0)
    ...
    def _update(self) -> None:
        ...
        if self._stopped:
            return
        connected = bool(getattr(self.ir, "is_connected", False))
        if not connected:
            self._update_disconnected_ui()
            self._after_job = self.root.after(400, self._update)
            return
        ...
        lap_target = self._feedback_lap_target()
        laps_used_label = "clean laps" if incident_free_only else "laps completed"
        ...
        self.card_total.set(f"{g_total:.2f}g", f'{usage_pct:.0f}% of the estimated limit')
        self.card_long.set(f"{long_g:+.2f}g", "brake + / throttle -")
        self.card_lat.set(f"{lat_g:+.2f}g", "left - / right +")
        self.card_limit.set(f"{self.estimated_limit_g:.2f}g", f"{len(coaching_laps)} {laps_used_label}")

        if self.external_reference_bins is not None:
            if coaching_ready:
                self.reference_var.set(f'IBT: {os.path.basename(self.external_reference_path or 'reference')}  •  ready')
            else:
                self.reference_var.set(f'IBT: {os.path.basename(self.external_reference_path or 'reference')}  •  missing {lap_target - len(coaching_laps)}')
        else:
            confident_bins = sum(1 for x in self.bin_confidence if x)
            self.reference_var.set(f'Live reference  •  {confident_bins}/{BINS_PER_LAP} reliable sections')
        ...
        if coaching_ready:
            ...
            if self.external_reference_bins is not None:
                sub = f'IBT loaded. Complete {remaining} {laps_used_label} to unlock the coaching.'
            else:
                sub = f'Complete {remaining} {laps_used_label} to build more confidence.'

        self.headline_var.set(headline)
        self.subheadline_var.set(
            f'{sub}  •  invalid laps: {self.invalid_laps_count}  •  outliers dropped: {self.outliers_removed_last}'
        )

        self._update_coach_cards(segments, laps_used_label, coaching_ready)
        self._draw_circle(long_g, lat_g, usage_pct)
        self._after_job = self.root.after(UPDATE_MS, self._update)

    def start(self) -> None:
        if not self._stopped and self._after_job is None:
            self._draw_circle(0.0, 0.0, 0.0)
            self._after_job = self.root.after(20, self._update)
    ...
    def stop(self) -> None:
        ...
        self._close_lapdist_overlay()
        if self.quickstart_window is not None and self.quickstart_window.winfo_exists():
            self.quickstart_window.destroy()
        self.quickstart_window = None
