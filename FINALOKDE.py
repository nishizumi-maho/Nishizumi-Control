    ...
    def _run(self):
        while not self._stop_event.wait(self.interval_s):
            ...
            try:
                ...
                print(f"[Watchdog:{self.name}] Wiederherstellung fehlgeschlagen: {exc}")
...
try:
    ...
    HAS_PYGAME = False
    print("Warnung: \u201epygame\u201c ist nicht installiert. Joystick-Unterst\u00fctzung deaktiviert.")
...
try:
    ...
    HAS_TTS = False
    print("Warnung: \u201epyttsx3\u201c ist nicht installiert. TTS deaktiviert.")
...
try:
    ...
    pyaudio = None
    HAS_PYAUDIO = False
    print("Warnung: \u201epyaudio\u201c ist nicht installiert. Auswahl an Audioger\u00e4ten begrenzt.")
...
try:
    ...
    sr = None
    HAS_SPEECH = False
    print("Warnung: \u201espeech_recognition\u201c ist nicht installiert. Sprachausl\u00f6ser deaktiviert.")
...
try:
    ...
    print(f"Warnung: 'vosk' nicht verf\u00fcgbar: {exc}")
...
def apply_app_icon(root: tk.Tk) -> None:
    ...
    for icon_name in ICON_CANDIDATES:
        ...
        try:
            ...
            print(f"[ICON] Fehler beim Laden von {icon_path}: {exc}")
...
def restart_program():
    ...
    try:
        ...
        print(f"[Neustart] Fehler beim Erzeugen eines neuen Prozesses: {exc}")
...
def set_startup_entry(enabled: bool) -> bool:
    ...
    try:
        ...
        print(f"[Startup] Eintrag konnte nicht aktualisiert werden: {exc}")
        return False
...
def mark_pending_scan(*, silent: bool = False) -> None:
    ...
    try:
        ...
        print(f"[PendingScan] Markierung konnte nicht beibehalten werden: {exc}")
...
def consume_pending_scan() -> Tuple[bool, bool]:
    ...
    try:
        ...
        print(f"[PendingScan] Markierung konnte nicht gelesen werden: {exc}")
    finally:
        try:
            os.remove(PENDING_SCAN_FILE)
        except Exception as exc:
            print(f"[PendingScan] Markierung konnte nicht gel\u00f6scht werden: {exc}")
...
if IS_WINDOWS:
    ...
    SendInput = None
    print("Warnung: Windows SendInput-APIs nicht verf\u00fcgbar; Eingabeinjektion deaktiviert.")
...
def press_key(scan_code: int):
    ...
    if SendInput is None:
        raise OSError("SendInput-APIs sind nur auf Windows-Plattformen verf\u00fcgbar.")
...
def _direct_pulse(scan_code: Optional[int], press_ms: int, interval_ms: int):
    ...
    try:
        ...
        print(f"[_direct_pulse] Fehler: {e}")
    ...
    with _TTS_LOCK:
        if _TTS_ENGINE is None:
            try:
                ...
                print(f"[TTS] Engine konnte nicht initialisiert werden: {exc}")
                _TTS_ENGINE = None
...
def _play_wave_file(path: str, output_device: Optional[int]) -> bool:
    ...
    try:
        ...
        print(f"[TTS] Audiowiedergabe fehlgeschlagen: {exc}")
        return False
...
def _tts_worker():
    ...
    while True:
        ...
        try:
            ...
            print(f"[TTS] Wiedergabe fehlgeschlagen: {exc}")
        finally:
            _TTS_QUEUE.task_done()
...
class InputManager:
    ...
    def __init__(self):
        ...
        if HAS_PYGAME:
            try:
                ...
                print(f"[InputManager] Pygame-Init-Fehler: {e}")
        ...
        if self.safe_mode:
            ...
            if HAS_PYGAME:
                try:
                    ...
                    print(f"[InputManager] Fehler beim Reaktivieren von Pygame: {e}")
        ...
        try:
            ...
            for i in range(count):
                try:
                    ...
                    devices.append((i, f"Ger\u00e4t {i} (Fehler)"))
                    
            return devices
        except Exception as e:
            print(f"[InputManager] Fehler beim Abrufen der Ger\u00e4te: {e}")
            return []
            ...
            for i in range(pygame.joystick.get_count()):
                ...
                if j.get_name() in self.allowed_devices:
                    try:
                        ...
                        print(f"[InputManager] Verbunden: {j.get_name()}")
                    except Exception:
                        pass
        except Exception:
            pass
    ...
    def _restart_input_loop(self):
        ...
        print("[InputManager][Watchdog] Eingabeschleife reagiert nicht, wird neu gestartet ...")
...
class VoiceListener:
    ...
    def _recover_listener(self):
        ...
        print("[Stimme][Watchdog] Listener-Thread reagiert nicht, wird neu gestartet ...")
        self.start()
    ...
    def _init_vosk_model(self, model_path: str):
        ...
        try:
            ...
            print(f"[Voice][Vosk] Modell konnte nicht geladen werden: {exc}")

    def _ensure_whisper_ready(self) -> bool:
        """Validate whisper.cpp prerequisites."""

        if not self.whisper_binary_path:
            self._whisper_error = "W\u00e4hlen Sie die ausf\u00fchrbare Datei whisper.cpp aus"
            return False

        if not os.path.exists(self.whisper_binary_path):
            self._whisper_error = "Die ausf\u00fchrbare Datei whisper.cpp wurde nicht gefunden"
            return False

        if not os.access(self.whisper_binary_path, os.X_OK):
            self._whisper_error = "whisper.cpp ist nicht ausf\u00fchrbar"
            return False

        if not self.whisper_model_path:
            self._whisper_error = "W\u00e4hlen Sie ein ggml/gguf-Modell (.bin/.gguf) aus."
            return False

        if not os.path.exists(self.whisper_model_path):
            self._whisper_error = "Modelldatei nicht gefunden"
            return False
    ...
    def _recognize_text(self, audio, recognizer=None) -> Optional[str]:
        ...
        if self.engine == "vosk" and HAS_VOSK and self.vosk_model:
            try:
                ...
                print(f"[Voice][Vosk] Erkennungsfehler: {exc}")
    ...
    def _transcribe_with_whisper(self, audio) -> Optional[str]:
        ...
        try:
            ...
            if result.returncode != 0:
                self._whisper_error = result.stderr.strip() or "whisper.cpp ist fehlgeschlagen"
                return None
            ...
            self._whisper_error = "Von whisper.cpp wurde kein Text zur\u00fcckgegeben"
            return None
        except Exception as exc:
            self._whisper_error = str(exc)
            print(f"[Stimme][Fl\u00fcstern] Erkennungsfehler: {exc}")
            return None
        finally:
            if tmp_path:
                try:
    ...
    def _listen_loop(self):
        ...
        try:
            ...
            print(f"[Stimme] H\u00f6rer hat angehalten: {exc}")
    ...
    def capture_once(
        ...
        if not self.available or sr is None:
            return None, "Spracherkennung nicht verf\u00fcgbar."
...
class VoiceTestDialog(tk.Toplevel):
    ...
    def __init__(
        ...
        self.title("Sprach- und Makrotest")
        self.geometry("430x360")

        info = tk.Label(
            self,
            text=(
                "Sagen Sie einen der konfigurierten Sätze, um das Makro auszulösen.\n"
                "Verwenden Sie die Testtaste, um sicherzustellen, dass Ihr Mikrofon und Ihre Phrasen"
                " funktionieren."
            ),
            wraplength=400,
            justify="left"
        ...
        ) or "Keine Phrasen konfiguriert."

        tk.Label(
            self,
            text="Verf\u00fcgbare Phrasen:",
            font=("Arial", 10, "bold")
        ...
        self.status_var = tk.StringVar(value="Warten auf Test...")
        self.heard_var = tk.StringVar(value="(noch nichts)")

        self.btn_listen = tk.Button(
            self,
            text="\ud83c\udfa4 H\u00f6ren Sie zu und testen Sie",
        ...
        tk.Label(manual, text="Phrase manuell ausf\u00fchren:").pack(
            anchor="w"
        )
    ...
    def start_listen(self):
        ...
        self.btn_listen.config(state="disabled", text="H\u00f6ren...")
        self.status_var.set("Sprechen Sie jetzt den konfigurierten Befehl...")
        self.heard_var.set("(H\u00f6ren)")
    ...
    def _listen_worker(self):
        ...
        def finalize():
            self.btn_listen.config(state="normal", text="\ud83c\udfa4 H\u00f6ren Sie zu und testen Sie")
            if error:
                self.status_var.set(f"Fehler beim Abh\u00f6ren: {error}")
                return

            if phrase is None:
                self.status_var.set("Stimme nicht verf\u00fcgbar.")
                return

            normalized = phrase.strip()
            self.heard_var.set(normalized or "(nichts erkannt)")

            if not normalized:
                self.status_var.set("Es wurde kein Satz erkannt.")
                return

            triggered = self._trigger_phrase(normalized)
            if triggered:
                self.status_var.set("Makro erfolgreich ausgel\u00f6st!")
            else:
                self.status_var.set("Phrase erkannt, aber kein Makro verkn\u00fcpft.")
    ...
    def run_manual_phrase(self):
        """Makro manuell über Texteingabe auslösen."""
        phrase = self.entry_manual.get().strip().lower()
        if not phrase:
            self.status_var.set("Geben Sie einen Ausdruck zum Testen ein.")
            return

        if self._trigger_phrase(phrase):
            self.status_var.set("Makro manuell ausgef\u00fchrt.")
        else:
            self.status_var.set("Mit dieser Phrase ist kein Makro verkn\u00fcpft.")
...
class DeviceSelector(tk.Toplevel):
    ...
    def __init__(self, parent, current_allowed: List[str], callback: Callable[[List[str]], None]):
        super().__init__(parent)
        self.title("USB-Ger\u00e4te verwalten")
        self.geometry("450x400")
        self.callback = callback

        tk.Label(
            self,
            text="W\u00e4hlen Sie aus, welche Ger\u00e4te die Anwendung verwenden kann",
            font=("Arial", 10, "bold"),
            pady=10
        ).pack()
        ...
        tk.Label(
            self,
            text="Aktivieren/deaktivieren Sie die Option, um die Ger\u00e4tenutzung zuzulassen bzw. zu verbieten",
            fg="gray"
        ).pack()
...
class OverlayConfigTab(tk.Frame):
    ...
    def __init__(self, parent, app):
        ...
        tk.Label(
            self.body,
            text="HUD-/Overlay-Konfiguration",
            font=("Arial", 11, "bold")
        ).pack(anchor="w", pady=(5, 5))

        # Global appearance settings
        appearance_frame = tk.LabelFrame(self.body, text="Globales HUD-Erscheinungsbild")
        ...
        tk.Label(appearance_frame, text="Schriftgr\u00f6\u00dfe:").grid(
        ...
        tk.Label(appearance_frame, text="Opazit\u00e4t:").grid(
        ...
        tk.Label(appearance_frame, text="Overlay-Breite:").grid(
        ...
        tk.Label(appearance_frame, text="Overlay-H\u00f6he:").grid(
        ...
        feedback_frame = tk.LabelFrame(
            self.body, text="Assist-Feedback-Schwellenwerte (pro Fahrzeug)"
        )
        feedback_frame.pack(fill="X", padx=5, pady=5)

        tk.Checkbutton(
            feedback_frame,
            text="Zeigen Sie ABS-/TC-/Schlupfhinweise auf dem HUD an",
        ...
        feedback_rows = [
            ("ABS l\u00e4nger aktiv als (s)", "abs_hold_s"),
            ("TC l\u00e4nger aktiv als (s)", "tc_hold_s"),
            ("Durchdrehschlupf (Verh\u00e4ltnis)", "wheelspin_slip"),
            ("Wheelspin-Dauer (s)", "wheelspin_hold_s"),
            ("Sperrbeleg (negativer Wert)", "lockup_slip"),
            ("Sperrdauer (s)", "lockup_hold_s"),
            ("Abklingzeit zwischen Warnungen", "cooldown_s"),
        ]
        ...
        variables_frame = tk.LabelFrame(
            self.body, 
            text="Anzuzeigende Variablen (pro Auto)"
        )
        variables_frame.pack(fill="beide", expand=True, padx=5, pady=5)

        header = tk.Frame(variables_frame)
        header.pack(fill="X", pady=(3, 3))
        
        tk.Label(header, text="Zeigen", width=8, anchor="w").pack(
            side="links", padx=2
        )
        tk.Label(header, text="Interner Name", width=25, anchor="w").pack(
            side="links", padx=2
        )
        ...
        tk.Label(
            self.body,
            text="Variablenauswahlen und Beschriftungen werden pro Auto gespeichert.\n"
                 "Darstellungseinstellungen gelten für alle Autos.",
            fg="gray",
            font=("Arial", 8)
    ...
    def pick_background_color(self):
        ...
        color = colorchooser.askcolor(title="Hintergrundfarbe")[1]
        if color:
    ...
    def pick_text_color(self):
        ...
        color = colorchooser.askcolor(title="Textfarbe")[1]
        if color:
    ...
    def _on_feedback_change(self, *_args):
        ...
        car = self.app.current_car or "Generisches Auto"
...
class GenericController:
    ...
    def _resolve_target(self, target: float) -> float:
        ...
        if abs(aligned - target) >= 0.0005:
            if self.update_status:
                self.update_status(f"Gerundet auf {aligned:.3f}", "orange")
            if self.app:
                short_name = self.var_name.replace("dc", "")
                self.app.notify_overlay_status(
                    f"{short_name}: Verwendung von {aligned:.3f} (am n\u00e4chsten)",
                    "orange"
                )
    ...
    def _run_target_loop(self):
        ...
        if not self.key_increase or not self.key_decrease:
            if self.update_status:
                self.update_status("Keine Tasten konfiguriert", "red")
            if self.app:
                self.app.notify_overlay_status(
                    f"{self.var_name.replace('dc', '')}: Keine Schl\u00fcssel",
                    "red"
                )
            return
        ...
        try:
            while True:
                ...
                if pending_target != active_request:
                    ...
                    if self.update_status:
                        self.update_status("Anpassen...", "orange")
                    if self.app:
                        self.app.notify_overlay_status(
                            f"Anpassen von {short_name} -> {active_target}",
                            "orange"
                        )
            ...
            print(f"[GenericController] Ausnahme: {exc}")
        finally:
            if success:
                ...
                if self.update_status:
                    self.update_status("Abgesagt", "red")
                if self.app:
                    self.app.notify_overlay_status(
                        f"{short_name} Abgebrochen",
                        "red"
                    )
            elif cleared:
                if self.update_status:
                ...
                if self.app:
                    self.app.notify_overlay_status(
                        f"{short_name} Gel\u00f6scht",
                        "orange"
                    )
            else:
                if self.update_status:
                    self.update_status("Fehlgeschlagen", "red")
                if self.app:
                    self.app.notify_overlay_status(
                        f"{short_name} Fehlgeschlagen",
                        "red"
                    )
    ...
    def find_minimum_effective_timing(
        ...
        """
        if not self.key_increase or not self.key_decrease:
            raise ValueError("Vor der Messung m\u00fcssen die Tasten zum Erh\u00f6hen/Verringern konfiguriert werden.")
...
class ControlTab(tk.Frame):
    ...
    def __init__(self, parent, controller: GenericController, label_name: str, app):
        ...
        tk.Button(
            keys_frame,
            text="Testen Sie die benutzerdefinierte Mindestzeit",
        ...
        # Status label
        self.lbl_status = tk.Label(body, text="Leerlauf", fg="gray")
        self.lbl_status.pack()

        # Presets/Macros
        presets_frame = tk.LabelFrame(
            body, 
            text="Voreinstellungen / Makros", 
            padx=5, 
            pady=5
        )
        presets_frame.pack(fill="beide", expand=True, padx=5, pady=5)

        tk.Label(
            presets_frame,
            text="RESET kehrt immer zu einem Basiswert zur\u00fcck (z. B. 0 oder 50). F\u00fcgen Sie unten Ihre gew\u00fcnschten Makrowerte hinzu.",
            fg="gray",
            font=("Arial", 8),
            wraplength=760,
            justify="left"
        ...
        tk.Label(
            presets_frame,
            text=(
                "Optionaler Sprachausl\u00f6ser: Geben Sie den genauen Satz ein, den Sie sagen m\u00f6chten, um das Makro auszuf\u00fchren."
                "Sprach-/Audioeinstellungen finden Sie unter Optionen \u2192 Sprach-/Audioeinstellungen."
            ),
            fg="gray",
            font=("Arial", 8),
            wraplength=760,
        ...
        tk.Label(
            header, text="Typ", width=6, anchor="w", font=("Arial", 8, "bold")
        ).pack(side="left")
        tk.Label(
            header,
            text="Makrowert",
    ...
    def run_bot_timing_probe(self):
        ...
        def _worker():
            try:
                ...
                self.after(
                    0,
                    lambda msg=error_msg: messagebox.showerror("Schl\u00fcssel fehlen", msg)
                )
                return
            ...
            if suggested is None:
                self.after(
                    ...
                    lambda: messagebox.showwarning(
                        "Sondenergebnis",
                        "Kein Timing innerhalb von 1\u2013120 ms, zuverl\u00e4ssig aktualisierte Telemetrie."
                    )
                )
            else:
                msg = (
                    f"Minimaler stabiler Impuls bei ~{suggested} ms erkannt.\n"
                    "Wenden Sie diesen Wert auf BOT-/benutzerdefinierte Timings an, um zuverlässige Updates zu erhalten."
                )
                self.after(0, lambda: messagebox.showinfo("Sondenergebnis", msg))
    ...
    def bind_game_key(self, direction: str):
        ...
        if self.app.app_state != "CONFIG":
            messagebox.showinfo("Beachten", "Rufen Sie zun\u00e4chst den CONFIG-Modus auf.")
            return
        ...
        btn.config(text="TASTE DR\u00dcCKEN...", bg="yellow")
    ...
    def _config_bind_button(self, button: tk.Button, data_store: Dict[str, Any]):
        ...
        def on_click():
            ...
            if code and code != "CANCEL":
                ...
                button.config(text="Binden festlegen", bg="#f0f0f0")
    ...
    def add_preset_row(
        ...
        label_text = "ZUR\u00dcCKSETZEN" if is_reset else "Macro"
        tk.Label(
            frame,
            text=label_text,
            width=6,
        ...
        bind_button = tk.Button(frame, text="Binden festlegen", width=12)
    ...
    def monitor_loop(self):
        ...
        try:
            self.lbl_monitor.config(text=f"Aktuell: {text}")
        except Exception:
            pass
    ...
    def set_config(self, config: Dict[str, Any]):
        ...
        self.btn_increase.config(text=config.get("key_increase_text", "Erh\u00f6hung (+) einstellen"))
        self.btn_decrease.config(text=config.get("key_decrease_text", "Verringerung (-) einstellen"))
...
class ComboTab(tk.Frame):
    ...
    def __init__(
        ...
        tk.Label(
            body,
            text="\u26a1 Kombi-Anpassungen (ein Ausl\u00f6ser -> mehrere Variablen)",
            fg="orange",
            font=("Arial", 10, "bold")
        ).pack(pady=5)
        ...
        tk.Label(
            body,
            text=(
                "Optionaler Sprachausl\u00f6ser: Geben Sie den genauen Satz ein, den Sie sagen m\u00f6chten, um diese Kombination auszul\u00f6sen."
                "Sprach-/Audioeinstellungen finden Sie unter Optionen \u2192 Sprach-/Audioeinstellungen."
            ),
            fg="gray",
            font=("Arial", 8),
    ...
    def add_dynamic_row(
        ...
        bind_button = tk.Button(
            frame,
            text="ZUR\u00dcCKSETZEN" if is_reset else "Binden festlegen",
            width=15,
            fg="red" if is_reset else "black"
        )
...
class GlobalTimingWindow(tk.Toplevel):
    ...
    def __init__(self, parent, callback_save: Callable):
        super().__init__(parent)
        self.title("Timing-Anpassungen (Anti-Erkennung)")
        ...
        # Profile selection
        profiles_frame = tk.LabelFrame(timing_frame, text="Verhaltensprofile")
        ...
        tk.Radiobutton(
            profiles_frame,
            text="\ud83e\udd16 BOT (experimentell, Verz\u00f6gerung nahe Null)",
        ...
        tk.Radiobutton(
            profiles_frame,
            text="\ud83e\udd16 BOT Stabil (schnell, zuverl\u00e4ssiger)",
            variable=self.var_profile,
            value="bot_safe",
            command=self._on_profile_change
        ).pack(anchor="w", padx=5, pady=2)
        ...
        tk.Radiobutton(
            profiles_frame,
            text="\ud83d\ude08 Aggressiv (schnell, roboterhaft)",
            variable=self.var_profile,
            value="aggressive",
            command=self._on_profile_change
        ).pack(anchor="w", padx=5, pady=2)
        ...
        tk.Radiobutton(
            profiles_frame,
            text="\ud83d\udee0 Benutzerdefiniert (Werte unten definieren)",
            variable=self.var_profile,
            value="custom",
            command=self._on_profile_change
        ...
        self.custom_frame = tk.LabelFrame(
            timing_frame, 
            text="Benutzerdefinierte Einstellungen (nur dieses Profil)"
        )
        self.custom_frame.pack(fill="X", padx=10, pady=10)

        tk.Label(self.custom_frame, text="Dr\u00fccken Sie Min (ms):").grid(
        ...
        tk.Label(self.custom_frame, text="Dr\u00fccken Sie Max (ms):").grid(
        ...
        tk.Label(self.custom_frame, text="Intervall Min (ms):").grid(
        ...
        tk.Label(self.custom_frame, text="Intervall Max (ms):").grid(
        ...
        self.check_random = tk.Checkbutton(
            self.custom_frame,
            text="Randomisieren (humanisieren)",
        ...
        tk.Label(self.custom_frame, text="Bereich (+/- ms):").grid(
        ...
        if profile == "custom":
            try:
                ...
                messagebox.showerror(
                    "Fehler", 
                    "Bitte verwenden Sie Zahlen nur im benutzerdefinierten Modus."
                )
                return
...
class iRacingControlApp:
    ...
    def __init__(self, root: tk.Tk):
        ...
        self.getting_started_text = (
            ...
            "1) W\u00e4hlen Sie Ihr Auto und Ihre Strecke."
            "2) Best\u00e4tigen Sie Ihre Eingabeger\u00e4te."
            "3) Steuerelemente des Scan-Treibers."
            "Wenn die Auto-/Streckenauswahl ausgegraut ist, \u00fcbernimmt iRacing die automatische Verwaltung"
            "Sie werden angezeigt, wenn Sie einer Sitzung beitreten. Nehmen Sie zun\u00e4chst an einer Sitzung teil, mit der Sie arbeiten m\u00f6chten"
            "Voreinstellungen festlegen oder \u201eWagen-/Streckenauswahl sperren\u201c auf der Registerkarte \u201eOptionen\u201c deaktivieren."
            "Verwenden Sie den CONFIG-Modus beim Wechseln der Bindung und den RUNNING-Modus beim Fahren."
        )
    ...
    def _apply_startup_preference(self, notify: bool = False) -> None:
        ...
        if notify:
            messagebox.showwarning(
                "Beginnen Sie mit Windows",
                "Der Windows-Starteintrag kann nicht aktualisiert werden."
                "Bitte \u00fcberpr\u00fcfen Sie die Berechtigungen oder versuchen Sie, als Administrator auszuf\u00fchren."
            )
    ...
    def _create_menu(self):
        ...
        options_menu.add_command(
            label="Timing-Anpassungen",
            command=self.open_timing_window
        )
        options_menu.add_command(
            label="Sprach-/Audioeinstellungen",
            command=self.open_voice_audio_settings
        )
        options_menu.add_separator()
        options_menu.add_command(
            label="Overlay ein-/ausblenden",
        ...
        options_menu.add_separator()
        options_menu.add_command(
            label="Standardeinstellungen wiederherstellen (Konfiguration l\u00f6schen)",
    ...
    def _create_main_ui(self):
        ...
        self.main_tabs.add(main_tab, text="\ud83c\udfc1 Haupts\u00e4chlich")
        self.main_tabs.add(options_tab, text="\u2699\ufe0f Optionen")
        ...
        presets_frame = tk.LabelFrame(
            steps_column,
            text="Schritt 1: W\u00e4hlen Sie Ihr Auto und Ihre Strecke"
        )
        ...
        tk.Label(selector_frame, text="Auto:").pack(side="left")
        ...
        tk.Label(selector_frame, text="Schiene:").pack(side="links")
        self.combo_track = ttk.Combobox(selector_frame, width=30)
        self.combo_track.pack(side="links", padx=5)
        ...
        devices_frame = tk.LabelFrame(
            steps_column,
            text="Schritt 2: Eingabeger\u00e4te best\u00e4tigen (Joystick/Rad)"
        )
        devices_frame.pack(fill="X", pady=(0, 8))

        self.check_safe = tk.Checkbutton(
            devices_frame,
            text="Nur-Tastatur-Modus (Neustart erforderlich)",
        ...
        scan_frame = tk.LabelFrame(
            steps_column,
            text="Schritt 3: Treiber-Steuerelemente scannen"
        )
        scan_frame.pack(fill="X")

        self.btn_scan = tk.Button(
            scan_frame,
            text="Scan-Steuerelemente f\u00fcr das ausgew\u00e4hlte Auto",
        ...
        tk.Label(
            scan_frame,
            text="Tipp: Scannen Sie, nachdem Sie Ger\u00e4te oder Voreinstellungen ge\u00e4ndert haben, um die Bindungen synchron zu halten.",
            fg="gray",
            font=("Arial", 9)
        ...
        options_notebook.add(general_tab, text="Allgemeine Einstellungen")

        general_container = tk.Frame(general_tab)
        general_container.pack(fill="beide", expand=True, padx=5, pady=5)
        general_container.columnconfigure(0, weight=1)

        general_left = tk.LabelFrame(
            general_container,
            text="Voreinstellungen und Ger\u00e4te"
        )
        general_left.grid(row=0, column=0, sticky="nsew", pady=(0, 8))

        tk.Checkbutton(
            general_left,
            text="Voreingestellte \u00c4nderungen automatisch speichern (Hotkeys/Makros)",
        ...
        tk.Checkbutton(
            general_left,
            text="Fahrzeug-/Streckenauswahl sperren (automatisch verwaltet)",
        ...
        tk.Checkbutton(
            general_left,
            text="Auto/Strecke automatisch \u00fcber iRacing erkennen",
        ...
        tk.Checkbutton(
            general_left,
            text="Automatischer Scan bei Fahrzeug-/Streckenwechsel",
        ...
        tk.Checkbutton(
            general_left,
            text="Popup \u201eErste Schritte\u201c beim Start anzeigen",
        ...
        stability_frame = tk.LabelFrame(
            general_container,
            text="Automatisierung und Verkn\u00fcpfungen"
        )
        stability_frame.grid(row=1, column=0, sticky="nsew")

        tk.Button(
            stability_frame,
            text="Timing-Anpassungen",
            command=self.open_timing_window
        ).pack(fill="X", padx=8, pady=(8, 6))

        tk.Button(
            stability_frame,
            text="Sprach-/Audiooptionen",
            command=self.open_voice_audio_settings
        ).pack(fill="X", padx=8, pady=(0, 6))

        tk.Checkbutton(
            stability_frame,
            text="Starten Sie neu, bevor Sie die Steuerelemente erneut scannen (nach dem ersten Scan).",
        ...
        tk.Checkbutton(
            stability_frame,
            text="Automatischer Neustart und Scan, wenn Sie einer Rennsitzung beitreten",
        ...
        tk.Checkbutton(
            stability_frame,
            text="Popup zum Abschluss des Scanvorgangs anzeigen",
        ...
        tk.Checkbutton(
            stability_frame,
            text="Versuchen Sie weiterhin, Hotkey-Ziele zu erreichen (kein Timeout)",
        ...
        tk.Label(
            clear_frame,
            text="Hotkey zum L\u00f6schen des Ziels (optional):"
        ).pack(side="links")
        self.btn_clear_target_bind = tk.Button(
            clear_frame,
            text="Legen Sie den Hotkey \u201eL\u00f6schen\u201c fest",
            width=18,
            command=self._set_clear_target_bind
        )
        self.btn_clear_target_bind.pack(side="links", padx=6)

        rescan_frame = tk.Frame(stability_frame)
        rescan_frame.pack(fill="X", padx=8, pady=(0, 8))
        tk.Label(
            rescan_frame,
            text="Manueller Rescan-Hotkey (Neustart + Scan + Voreinstellung laden):"
        ).pack(side="links")
        self.btn_manual_rescan_bind = tk.Button(
            rescan_frame,
            text="Stellen Sie den Rescan-Hotkey ein",
            width=18,
    ...
    def open_getting_started_window(self) -> None:
        ...
        self.getting_started_window.title("Erste Schritte")
    ...
    def _list_microphones(self) -> List[Tuple[int, str]]:
        devices: List[Tuple[int, str]] = [(-1, "Systemstandard")]
        if not HAS_SPEECH:
            return devices
        ...
        try:
            ...
            print(f"[Stimme] Mikrofone k\u00f6nnen nicht aufgelistet werden: {exc}")
            ...
            try:
                for idx in range(pa.get_device_count()):
                    ...
                    if info.get("maxOutputChannels", 0) > 0:
                        name = info.get("name", f"Ausgabe {idx}")
                        devices.append((idx, name))
            finally:
                pa.terminate()
        except Exception as exc:  # noqa: PERF203
            print(f"[Audio] Ausgabeger\u00e4te k\u00f6nnen nicht aufgelistet werden: {exc}")
    ...
    def _refresh_audio_device_lists(self):
        ...
        if self.mic_combo:
            ...
            current_label = self._device_label(
                ...
                dict(mic_devices).get(self.microphone_device.get(), "Systemstandard")
            )
        ...
        if self.audio_output_combo:
            ...
            current_output_label = self._device_label(
                ...
                dict(output_devices).get(self.audio_output_device.get(), "Systemstandard")
            )
    ...
    def open_voice_audio_settings(self):
        ...
        self.voice_window.title("Sprach- und Audiooptionen")
        ...
        notebook.add(voice_tab, text="Stimme/Audio")
    ...
    def close_voice_audio_window(self) -> None:
        """Schließen und bereinigen Sie das Fenster mit den Sprach-/Audiooptionen."""
    ...
    def _build_voice_audio_tab(self, parent: tk.Widget):
        ...
        if not HAS_SPEECH:
            tk.Label(
                toggles_frame,
                text="(Installieren Sie \u201espeech_recognition\u201c f\u00fcr Stimme)",
                fg="gray",
                font=("Arial", 8)
            ).pack(side="left", padx=4)

        engine_frame = tk.LabelFrame(parent, text="Erkennungsmaschine")
        engine_frame.pack(fill="X", padx=2, pady=6)

        engine_row = tk.Frame(engine_frame)
        engine_row.pack(fill="X", padx=6, pady=2)

        ttk.Label(engine_row, text="Sprachmotor:").pack(side="left", padx=4)
        ...
        self.btn_vosk_model = tk.Button(
            model_row,
            text="Vosk-Modell ausw\u00e4hlen...",
            command=self.choose_vosk_model
        )
        self.btn_vosk_model.pack(side="links", padx=4)

        self.btn_whisper_binary = tk.Button(
            model_row,
            text="W\u00e4hlen Sie whisper.cpp...",
            command=self.choose_whisper_binary
        )
        self.btn_whisper_binary.pack(side="links", padx=4)

        self.btn_whisper_model = tk.Button(
            model_row,
            text="Whisper-Modell ausw\u00e4hlen...",
        ...
        device_frame = tk.LabelFrame(parent, text="Eingabe-/Ausgabeger\u00e4te")
        device_frame.pack(fill="X", padx=2, pady=6)

        mic_row = tk.Frame(device_frame)
        mic_row.pack(fill="X", padx=6, pady=2)

        ttk.Label(mic_row, text="Mikrofon:").pack(side="left")
        ...
        ttk.Label(out_row, text="Audioausgabe (TTS):").pack(side="left")
        ...
        tuning_frame = tk.LabelFrame(
            parent,
            text="Stimmabstimmung (Genauigkeit und Geschwindigkeit)"
        )
        ...
        ttk.Label(tuning_row_1, text="Umgebungsger\u00e4usch(e):").pack(side="links")
        ttk.Spinbox(
            tuning_row_1,
            from_=0.0,
            to=3.0,
        ...
        ttk.Label(tuning_row_1, text="Maximale Phrasendauer (s):").pack(side="links")
        ttk.Spinbox(
            tuning_row_1,
            from_=0.2,
            to=6.0,
        ...
        tk.Checkbutton(
            tuning_row_1,
            text="Dynamische Energie (automatisch)",
            variable=self.voice_dynamic_energy
        ).pack(side="links", padx=8)

        tuning_row_2 = tk.Frame(tuning_frame)
        tuning_row_2.pack(fill="X", padx=6, pady=2)

        ttk.Label(tuning_row_2, text="Anf\u00e4ngliches Timeout (s):").pack(side="links")
        ttk.Spinbox(
            tuning_row_2,
            from_=0.0,
            to=5.0,
        ...
        ttk.Label(tuning_row_2, text="Kontinuierliche Zeit\u00fcberschreitung (s):").pack(side="links")
        ttk.Spinbox(
            tuning_row_2,
            from_=0.0,
            to=5.0,
        ...
        ttk.Label(tuning_row_2, text="Mindestenergie:").pack(side="links")
        ttk.Entry(
            tuning_row_2,
            width=8,
    ...
    def toggle_mode(self):
        ...
        if self.app_state == "RUNNING":
            ...
            self.btn_mode.config(
                text="Modus: CONFIG (Zum Speichern und Ausf\u00fchren anklicken)",
                bg="orange"
            )
            ...
            self.btn_mode.config(text="Modus: LAUFEN", bg="#90ee90")
    ...
    def clear_all_targets(self):
        ...
        self.notify_overlay_status("Ziele gel\u00f6scht", "orange")
    ...
    def trigger_safe_mode_update(self):
        """Lösen Sie die Aktualisierung des abgesicherten Modus mit einem Neustart aus."""
        new_value = self.use_keyboard_only.get()
        if messagebox.askokcancel(
            "Neustart erforderlich",
            "Um den Nur-Tastatur-Modus anzuwenden, ist ein Neustart erforderlich. Best\u00e4tigen?"
        ):
            self.save_config()
            restart_program()
        else:
    ...
    def open_device_manager(self):
        ...
        if self.use_keyboard_only.get():
            messagebox.showinfo(
                "Tastaturmodus",
                "Deaktivieren Sie den \u201eNur-Tastatur-Modus\u201c, um Joystick-Ger\u00e4te zu verwalten."
            )
            return
    ...
    def action_save_preset(self):
        ...
        if not car or not track:
            messagebox.showwarning("Fehler", "Definieren Sie Auto und Strecke.")
            return
    ...
    def _save_preset_for_pair(
        ...
        self.update_preset_ui()
        if show_message:
            messagebox.showinfo("Gespeichert", f"Voreinstellung gespeichert f\u00fcr {car} @ {track}")
    ...
    def load_specific_preset(self, car: str, track: str):
        ...
        self.register_current_listeners()
        print(f"[Voreinstellung] Geladen {car} / {track}")
    ...
    def action_clear_preset(self):
        ...
        if car in self.saved_presets and track in self.saved_presets[car]:
            if not messagebox.askyesno(
                "Best\u00e4tigen", 
                f"Voreinstellung f\u00fcr {car} @ {track} l\u00f6schen?"
            ):
                return
    ...
    def scan_driver_controls(
        ...
        *,
        ...
        try:
            ...
            print(f"[Scan] Unerwarteter Fehler: {exc}")
            if not silent_if_unavailable:
                messagebox.showerror(
                    "Scan",
                    "Der Scanvorgang ist fehlgeschlagen. Bitte versuchen Sie es erneut, sobald Sie in der Sitzung sind."
                )
            if on_complete:
                self.root.after(0, on_complete)
            self._scan_in_progress = False
    ...
    def _scan_driver_controls_worker(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                print(f"[Scan] Fehler beim Lesen der Variablen: {e}")
    ...
    def _finish_scan_driver_controls(
        ...
        if status == "unavailable":
            if not silent_if_unavailable:
                messagebox.showerror(
                    "Fehler",
                    "\u00d6ffnen Sie iRacing (oder geben Sie eine Sitzung ein)."
                )
            if on_complete:
                on_complete()
            return
        ...
        if status == "no_candidates":
            messagebox.showwarning(
                "Scan",
                "SDK hat noch keine Variablen zur\u00fcckgegeben."
                "Steigen Sie in das Auto ein (Fahren), passen Sie die Steuerung an und versuchen Sie es erneut."
            )
            if on_complete:
                on_complete()
            return
        ...
        if status == "no_vars":
            messagebox.showwarning(
                "Scan",
                "Keine numerischen \u201edc*\u201c-Variablen gefunden."
                "M\u00f6glicherweise verf\u00fcgt das Auto nicht \u00fcber eine Fahrersteuerung oder Sie befinden sich nicht im Fahrmodus."
            )
            if on_complete:
                on_complete()
            return
        ...
        if status != "ok":
            print(f"[Scan] Unerwarteter Fehler: {result.get('error')}")
            if not silent_if_unavailable:
                messagebox.showerror(
                    "Scan",
                    "Der Scanvorgang ist fehlgeschlagen. Bitte versuchen Sie es erneut, sobald Sie in der Sitzung sind."
        ...
        if self.show_scan_popup.get():
            messagebox.showinfo(
                "Scan",
                f"{len(clean_vars)} \u201eDC\u201c-Steuerungen f\u00fcr dieses Auto konfiguriert."
            )
        if on_complete:
            on_complete()
    ...
    def rebuild_tabs(self, vars_list: List[Tuple[str, bool]]):
        ...
        self.notebook.add(combo_frame, text="⚡ Kombos")
        ...
        self.notebook.add(overlay_frame, text="HUD / Einblendung")

        # Load overlay for current car
        car = self.current_car or "Generisches Auto"
    ...
    def notify_overlay_status(self, text: str, color: str):
        ...
        self.ui(
            self.root.after,
            2000,
            lambda: self.overlay.update_status_text("HUD bereit", "white")
        )
    ...
    def _update_overlay_feedback(self):
        ...
        if state["abs_active"] >= cfg["abs_hold_s"]:
            self._push_overlay_alert(
                "ABS zu lange aktiv: Bremse l\u00f6sen oder ABS absenken.",
                "orange",
                cfg,
                now
            )
        ...
        if state["tc_active"] >= cfg["tc_hold_s"]:
            self._push_overlay_alert(
                "TC l\u00f6st st\u00e4ndig aus: Erw\u00e4gen Sie eine Verringerung des TC oder einen Wechsel der Karte.",
                "orange",
                cfg,
                now
            )
        ...
        if state["spin_active"] >= cfg["wheelspin_hold_s"]:
            self._push_overlay_alert(
                "Durchdrehende R\u00e4der erkannt: TC erh\u00f6hen oder Gas geben.",
                "orange",
                cfg,
                now
            )
        ...
        if state["lock_active"] >= cfg["lockup_hold_s"]:
            self._push_overlay_alert(
                "Blockierung erkannt: ABS erh\u00f6hen oder Pedaldruck verringern.",
                "orange",
                cfg,
                now
            )
    ...
    def save_config(self):
        ...
        try:
            ...
            print(f"[SAVE] Fehler beim Speichern der Konfiguration: {e}")
    ...
    def _format_vosk_status(self) -> str:
        ...
        if engine != "vosk":
            if model_path:
                return f"Ausgew\u00e4hltes Vosk-Modell: {model_name or model_path}"
            return "Verwendung der Windows-Spracherkennung"

        if not HAS_VOSK:
            if VOSK_IMPORT_ERROR:
                return f"Vosk nicht verf\u00fcgbar: {VOSK_IMPORT_ERROR}"
            return "Vosk nicht installiert"

        if not model_path:
            return "W\u00e4hlen Sie einen Vosk-Modellordner aus"

        if voice_listener._vosk_error:
            return f"Modellfehler: {voice_listener._vosk_error}"

        if voice_listener.vosk_model is not None:
            name = model_name or model_path
            return f"Vosk bereit: {name}"
    ...
    def _format_whisper_status_details(self) -> str:
        ...
        if not self.whisper_binary_path.get():
            if self.whisper_model_path.get():
                ...
                return f"Ausgew\u00e4hltes Whisper-Modell: {model_name}"
            return "W\u00e4hlen Sie die ausf\u00fchrbare Datei whisper.cpp aus"

        if not os.path.exists(self.whisper_binary_path.get()):
            return "Ausf\u00fchrbare Datei nicht gefunden"

        if not self.whisper_model_path.get():
            exe_name = os.path.basename(self.whisper_binary_path.get())
            return f"whisper.cpp ausgew\u00e4hlt: {exe_name} (Modell ausw\u00e4hlen)"

        if not os.path.exists(self.whisper_model_path.get()):
            return "Modell nicht gefunden"

        if voice_listener._whisper_error:
            return f"Fl\u00fcsterfehler: {voice_listener._whisper_error}"

        model_name = os.path.basename(self.whisper_model_path.get())
        exe_name = os.path.basename(self.whisper_binary_path.get())
        return f"Fl\u00fcsterbereit: {exe_name} | {model_name}"
    ...
    def choose_vosk_model(self):
        ...
        path = filedialog.askdirectory(title="W\u00e4hlen Sie den Vosk-Modellordner aus")
        if not path:
            return
    ...
    def choose_whisper_binary(self):
        ...
        path = filedialog.askopenfilename(
            title="W\u00e4hlen Sie die ausf\u00fchrbare Datei whisper.cpp aus",
            filetypes=[("Ausführbare Datei", "*"), ("Alle Dateien", "*")]
        )
        if not path:
            return
    ...
    def choose_whisper_model(self):
        ...
        path = filedialog.askopenfilename(
            title="Whisper-Modell ausw\u00e4hlen (.bin/.gguf)",
            filetypes=[("Whisper-Modelle", "*.bin *.gguf"), ("Alle Dateien", "*")]
        )
        if not path:
            return
    ...
    def open_voice_test_dialog(self):
        ...
        if not HAS_SPEECH:
            messagebox.showinfo(
                ...
                "Installieren Sie das Paket \u201espeech_recognition\u201c, um die Sprachsteuerung zu aktivieren."
            )
            return
        ...
        if not phrases_map:
            messagebox.showinfo(
                ...
                "F\u00fcgen Sie den Registerkarten Phrasen hinzu, um Sprachbefehle zu testen."
            )
            return
    ...
    def restore_defaults(self):
        ...
        if not messagebox.askyesno(
            ...
            "Dadurch wird Ihre Konfigurationsdatei gel\u00f6scht und die App neu gestartet. Weitermachen?"
        ):
            return
        ...
        try:
            ...
            messagebox.showerror(
                "Fehler",
                f"Konfiguration konnte nicht gel\u00f6scht werden: {exc}"
            )
            return

        messagebox.showinfo(
            "Standardeinstellungen wiederhergestellt",
            "Konfiguration zur\u00fcckgesetzt. Die Anwendung wird jetzt neu gestartet."
        )
        restart_program()
...
def main():
    ...
    try:
        ...
        import traceback
        traceback.print_exc()
        input("Dr\u00fccken Sie die Eingabetaste, um zu schlie\u00dfen...")
