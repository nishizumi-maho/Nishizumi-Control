    ...
    def _run(self):
        while not self._stop_event.wait(self.interval_s):
            ...
            try:
                ...
                print(f"[Watchdog:{self.name}] error de recuperación: {exc}")
...
try:
    ...
    HAS_PYGAME = False
    print("Advertencia: 'pygame' no está instalado. Soporte de joystick desactivado.")
...
try:
    ...
    HAS_TTS = False
    print("Advertencia: 'pyttsx3' no instalado. TTS deshabilitado.")
...
try:
    ...
    pyaudio = None
    HAS_PYAUDIO = False
    print("Advertencia: 'pyaudio' no está instalado. Selección de dispositivo de audio limitada.")
...
try:
    ...
    sr = None
    HAS_SPEECH = False
    print("Advertencia: 'speech_recognition' no está instalado. Activadores de voz desactivados.")
...
try:
    ...
    print(f"Advertencia: 'vosk' no disponible: {exc}")
...
def apply_app_icon(root: tk.Tk) -> None:
    ...
    for icon_name in ICON_CANDIDATES:
        ...
        try:
            ...
            print(f"[ICON] Fallido to load {icon_path}: {exc}")
...
def restart_program():
    ...
    try:
        ...
        print(f"[Reiniciar] No se pudo iniciar un nuevo proceso: {exc}")
...
def set_startup_entry(enabled: bool) -> bool:
    ...
    try:
        ...
        print(f"[Inicialización] No se pudo actualizar la entrada: {exc}")
        return False
...
def mark_pending_scan(*, silent: bool = False) -> None:
    ...
    try:
        ...
        print(f"[PendingVerificación] No se pudo conservar el marcador: {exc}")
...
def consume_pending_scan() -> Tuple[bool, bool]:
    ...
    try:
        ...
        print(f"[PendingVerificación] No se pudo leer el marcador: {exc}")
    finally:
        try:
            os.remove(PENDING_SCAN_FILE)
        except Exception as exc:
            print(f"[PendingVerificación] No se pudo borrar el marcador: {exc}")
...
if IS_WINDOWS:
    ...
    SendInput = None
    print("Advertencia: Las API SendInput de Windows no están disponibles; Inyección de entrada deshabilitada.")
...
def press_key(scan_code: int):
    ...
    if SendInput is None:
        raise OSError("Las API de SendInput solo están disponibles en plataformas Windows.")
    ...
    with _TTS_LOCK:
        if _TTS_ENGINE is None:
            try:
                ...
                print(f"[TTS] error al inicializar el motor: {exc}")
                _TTS_ENGINE = None
...
def _play_wave_file(path: str, output_device: Optional[int]) -> bool:
    ...
    try:
        ...
        print(f"[TTS] error en la reproducción de audio: {exc}")
        return False
...
def _tts_worker():
    ...
    while True:
        ...
        try:
            ...
            print(f"[TTS] Falló la reproducción: {exc}")
        finally:
            _TTS_QUEUE.task_done()
...
class InputManager:
    ...
    def __init__(self):
        ...
        if HAS_PYGAME:
            try:
                ...
                print(f"[InputManager] error al iniciar pygame: {e}")
        ...
        if self.safe_mode:
            ...
            if HAS_PYGAME:
                try:
                    ...
                    print(f"[InputManager] error al reactivar pygame: {e}")
        ...
        try:
            ...
            for i in range(count):
                try:
                    ...
                    devices.append((i, f"Dispositivo {i} (error)"))
                    
            return devices
        except Exception as e:
            print(f"[InputManager] error al obtener dispositivos: {e}")
            return []
    ...
    def _restart_input_loop(self):
        ...
        print("[InputManager][Watchdog] Bucle de entrada que no responde, reiniciando...")
...
class VoiceListener:
    ...
    def _recover_listener(self):
        ...
        print("[Voz][Watchdog] El hilo de escucha no responde, se reinicia...")
        self.start()
    ...
    def _init_vosk_model(self, model_path: str):
        """Cargar the Vosk model from disk if available."""
        ...
        try:
            ...
            print(f"[Voz][Vosk] error al cargar el modelo: {exc}")

    def _ensure_whisper_ready(self) -> bool:
        """Validate whisper.cpp prerequisites."""

        if not self.whisper_binary_path:
            self._whisper_error = "Seleccione el ejecutable whisper.cpp"
            return False

        if not os.path.exists(self.whisper_binary_path):
            self._whisper_error = "No se encontró el ejecutable de Whisper.cpp"
            return False

        if not os.access(self.whisper_binary_path, os.X_OK):
            self._whisper_error = "whisper.cpp no es ejecutable"
            return False

        if not self.whisper_model_path:
            self._whisper_error = "Seleccione una plantilla ggml/gguf (.bin/.gguf)"
            return False

        if not os.path.exists(self.whisper_model_path):
            self._whisper_error = "Archivo de plantilla no encontrado"
            return False
    ...
    def _recognize_text(self, audio, recognizer=None) -> Optional[str]:
        ...
        if self.engine == "vosk" and HAS_VOSK and self.vosk_model:
            try:
                ...
                print(f"[Voz][Vosk] error de reconocimiento: {exc}")
    ...
    def _transcribe_with_whisper(self, audio) -> Optional[str]:
        ...
        try:
            ...
            if result.returncode != 0:
                self._whisper_error = result.stderr.strip() or "whisper.cpp falló"
                return None
            ...
            self._whisper_error = "No se devolvió ningún texto por whisper.cpp"
            return None
        except Exception as exc:
            self._whisper_error = str(exc)
            print(f"[Voz][Whisper] Error de reconocimiento: {exc}")
            return None
        finally:
            if tmp_path:
                try:
    ...
    def _listen_loop(self):
        ...
        try:
            ...
            print(f"[Voz] Oyente interrumpido: {exc}")
    ...
    def capture_once(
        ...
        if not self.available or sr is None:
            return None, "Reconocimiento de voz no disponible."
...
class VoiceTestDialog(tk.Toplevel):
    ...
    def __init__(
        ...
        self.title("Prueba de voz y macros")
        ...
        info = tk.Label(
            ...
            text=(
                ...
                "Utilice el botón de prueba para asegurarse de que su micrófono y sus frases estén"
                " laboral."
            ),
            wraplength=400,
            justify="left"
        ...
        self.status_var = tk.StringVar(value="Esperando prueba...")
        self.heard_var = tk.StringVar(value="(todavía nada)")

        self.btn_listen = tk.Button(
            self,
            text="🎤 Escuche y pruebe",
        ...
        tk.Label(manual, text="Ejecutar sentencia manualmente:").pack(
            anchor="w"
        )
    ...
    def start_listen(self):
        ...
        self.btn_listen.config(state="discapacitado", text="Escuchando...")
        self.status_var.set("Di el comando configurado ahora...")
        self.heard_var.set("(escuchando)")
    ...
    def _listen_worker(self):
        ...
        def finalize():
            self.btn_listen.config(state="normales", text="🎤 Escuche y pruebe")
            if error:
                self.status_var.set(f"error al escuchar: {error}")
                return

            if phrase is None:
                self.status_var.set("Voz no disponible.")
                return

            normalized = phrase.strip()
            self.heard_var.set(normalized or "(nada reconocido)")

            if not normalized:
                self.status_var.set("No se reconocieron frases.")
                return

            triggered = self._trigger_phrase(normalized)
            if triggered:
                self.status_var.set("Macro activada exitosamente!")
            else:
                self.status_var.set("Frase reconocida, pero no hay macros vinculadas.")
    ...
    def run_manual_phrase(self):
        """Activar macro manualmente desde la entrada de texto."""
        phrase = self.entry_manual.get().strip().lower()
        if not phrase:
            self.status_var.set("Ingrese una frase para probar.")
            return

        if self._trigger_phrase(phrase):
            self.status_var.set("Macro ejecutada manualmente.")
        else:
            self.status_var.set("No hay macros vinculadas a esta oración.")
...
class DeviceSelector(tk.Toplevel):
    ...
    def __init__(self, parent, current_allowed: List[str], callback: Callable[[List[str]], None]):
        super().__init__(parent)
        self.title("Administrar dispositivos USB")
        self.geometry("450x400")
        self.callback = callback

        tk.Label(
            self,
            text="Seleccione qué dispositivos puede usar la aplicación",
            font=("Arial", 10, "bold"),
            pady=10
        ).pack()
        ...
        tk.Label(
            self,
            text="Marque/desmarque para permitir/no permitir el uso del dispositivo",
            fg="gray"
        ).pack()
...
class OverlayConfigTab(tk.Frame):
    ...
    def __init__(self, parent, app):
        ...
        tk.Label(
            self.body,
            text="Configuración de HUD/superposición",
            font=("Arial", 11, "bold")
        ).pack(anchor="w", pady=(5, 5))

        # Global appearance settings
        appearance_frame = tk.LabelFrame(self.body, text="Apariencia global del HUD")
        ...
        tk.Label(appearance_frame, text="Tamaño de fuente:").grid(
        ...
        tk.Label(appearance_frame, text="Opacidad:").grid(
        ...
        tk.Label(appearance_frame, text="Ancho de superposición:").grid(
        ...
        tk.Label(appearance_frame, text="Altura de superposición:").grid(
        ...
        feedback_frame = tk.LabelFrame(
            self.body, text="Umbrales de retroalimentación de asistencia (por automóvil)"
        )
        feedback_frame.pack(fill="x", padx=5, pady=5)

        tk.Checkbutton(
            feedback_frame,
            text="Mostrar consejos de ABS/TC/deslizamiento en HUD",
        ...
        feedback_rows = [
            ("ABS activo durante más de (s)", "abs_hold_s"),
            ("TC activo por más de (s)", "tc_hold_s"),
            ("Patinaje de las ruedas (motivo)", "wheelspin_slip"),
            ("Duración(es) del patinaje", "wheelspin_hold_s"),
            ("Deslizamiento de bloqueo (valor negativo)", "lockup_slip"),
            ("Duración(es) del bloqueo", "lockup_hold_s"),
            ("Intervalo entre alertas", "cooldown_s"),
        ]
        ...
        variables_frame = tk.LabelFrame(
            self.body, 
            text="Variables a mostrar (por auto)"
        )
        ...
        tk.Label(header, text="Ver", width=8, anchor="w").pack(
            side="left", padx=2
        )
        tk.Label(header, text="Nombre interno", width=25, anchor="w").pack(
            side="left", padx=2
        )
        tk.Label(header, text="etiqueta HUD", width=20, anchor="w").pack(
            side="left", padx=2
        )
    ...
    def pick_text_color(self):
        ...
        color = colorchooser.askcolor(title="Color del texto")[1]
        if color:
    ...
    def _load_feedback_for_car(self, car_name: str) -> None:
        """Cargar per-car feedback thresholds into the UI fields."""
...
class GenericController:
    ...
    def _resolve_target(self, target: float) -> float:
        ...
        if abs(aligned - target) >= 0.0005:
            if self.update_status:
                self.update_status(f"Redondeado a {aligned:.3f}", "naranja")
            if self.app:
    ...
    def clear_target(self):
        """Borrar any pending target requests and stop adjusting."""
        with self._target_lock:
    ...
    def _run_target_loop(self):
        ...
        if not self.key_increase or not self.key_decrease:
            if self.update_status:
                self.update_status("No hay claves configuradas", "rojo")
            if self.app:
                self.app.notify_overlay_status(
        ...
        try:
            while True:
                ...
                if pending_target != active_request:
                    ...
                    if self.update_status:
                        self.update_status("Ajustando...", "naranja")
                    if self.app:
                        self.app.notify_overlay_status(
            ...
            print(f"[GenericController] Excepción: {exc}")
        finally:
            if success:
                message = f"{short_name} OK ({active_target})"
                if self.update_status:
                    self.update_status("Listo", "verde")
                if self.app:
                ...
                if self.update_status:
                    self.update_status("Cancelado", "rojo")
                if self.app:
                    self.app.notify_overlay_status(
                        f"{short_name} Cancelado",
                        "red"
                    )
            elif cleared:
                if self.update_status:
                ...
                if self.app:
                    self.app.notify_overlay_status(
                        f"{short_name} Borrared",
                        "orange"
                    )
            else:
                if self.update_status:
                    self.update_status("Fallido", "rojo")
                if self.app:
                    self.app.notify_overlay_status(
                        f"{short_name} Fallido",
                        "red"
                    )
    ...
    def find_minimum_effective_timing(
        ...
        """
        if not self.key_increase or not self.key_decrease:
            raise ValueError("Las teclas de aumento/disminución deben configurarse antes de realizar la comprobación.")
...
class ControlTab(tk.Frame):
    ...
    def __init__(self, parent, controller: GenericController, label_name: str, app):
        ...
        tk.Button(
            keys_frame,
            text="Pruebe el tiempo mínimo personalizado",
        ...
        # Status label
        self.lbl_status = tk.Label(body, text="inactivo", fg="gris")
        self.lbl_status.pack()

        # Presets/Macros
        presets_frame = tk.LabelFrame(
            body, 
            text="Preajustes/Macros", 
            padx=5, 
            pady=5
        )
        ...
        tk.Label(
            presets_frame,
            text="REINICIAR siempre regresa a un valor base (por ejemplo, 0 o 50). Agregue los valores de macro que más utiliza a continuación.",
            fg="gray",
            font=("Arial", 8),
            wraplength=760,
            justify="left"
        ...
        tk.Label(
            presets_frame,
            text=(
                "Activador de voz opcional: ingrese la frase exacta que dirá para ejecutar la macro."
                "La configuración de voz/audio se encuentra en Opciones → Configuración de voz/audio."
            ),
            fg="gray",
            font=("Arial", 8),
            wraplength=760,
        ...
        tk.Label(
            header, text="Tipo", width=6, anchor="w", font=("arial", 8, "negrita")
        ).pack(side="left")
        tk.Label(
            header,
            text="Valor macro",
        ...
        tk.Label(
            header,
            text="Frase de activación de voz",
            width=18,
            anchor="w",
            font=("Arial", 8, "bold")
        ).pack(side="left", padx=5)
    ...
    def run_bot_timing_probe(self):
        ...
        def _worker():
            try:
                ...
                self.after(
                    0,
                    lambda msg=error_msg: messagebox.showerror("llaves faltantes", msg)
                )
                return
            ...
            if suggested is None:
                self.after(
                    ...
                    lambda: messagebox.showwarning(
                        "comprobar resultado",
                        "Ningún tiempo entre 1 y 120 ms actualizó de manera confiable la telemetría."
                    )
                )
            else:
                msg = (
                ...
                self.after(0, lambda: messagebox.showinfo("comprobar resultado", msg))
    ...
    def bind_game_key(self, direction: str):
        ...
        if self.app.app_state != "CONFIG":
            messagebox.showinfo("Advertencia", "Ingrese primero al modo CONFIG.")
            return
        ...
        btn.config(text="PRESIONA LA TECLA...", bg="amarillo")
    ...
    def _config_bind_button(self, button: tk.Button, data_store: Dict[str, Any]):
        ...
        def on_click():
            ...
            if code and code != "CANCEL":
                ...
                button.config(text="Establecer acceso directo", bg="#f0f0f0")
    ...
    def add_preset_row(
        ...
        label_text = "REINICIAR" if is_reset else "Macro"
        tk.Label(
            frame,
            text=label_text,
            width=6,
        ...
        bind_button = tk.Button(frame, text="Establecer acceso directo", width=12)
    ...
    def monitor_loop(self):
        ...
        try:
            self.lbl_monitor.config(text=f"Actual: {text}")
        except Exception:
            pass
    ...
    def set_config(self, config: Dict[str, Any]):
        ...
        self.btn_increase.config(text=config.get("key_increase_text", "Establecer aumento (+)"))
        self.btn_decrease.config(text=config.get("key_decrease_text", "Establecer disminución (-)"))
...
class ComboTab(tk.Frame):
    ...
    def __init__(
        ...
        tk.Label(
            body,
            text="⚡ Ajustes combinados (un disparador -> múltiples variables)",
            fg="orange",
            font=("Arial", 10, "bold")
        ).pack(pady=5)
        ...
        tk.Label(
            body,
            text=(
                "Activador de voz opcional: ingresa la frase exacta que dirás para activar este combo."
                "La configuración de voz/audio se encuentra en Opciones → Configuración de voz/audio."
            ),
            fg="gray",
            font=("Arial", 8),
    ...
    def add_dynamic_row(
        ...
        bind_button = tk.Button(
            frame,
            text="REINICIAR" if is_reset else "Establecer acceso directo",
            width=15,
            fg="red" if is_reset else "black"
        )
...
class GlobalTimingWindow(tk.Toplevel):
    ...
    def __init__(self, parent, callback_save: Callable):
        super().__init__(parent)
        self.title("Ajustes de sincronización (anti-detección)")
        ...
        notebook.add(timing_frame, text="Sincronización")

        # Profile selection
        profiles_frame = tk.LabelFrame(timing_frame, text="Perfiles de comportamiento")
        ...
        tk.Radiobutton(
            profiles_frame,
            text="🤖 BOT (experimental, casi cero retraso)",
        ...
        tk.Radiobutton(
            profiles_frame,
            text="🤖 BOT estable (rápido, más confiable)",
            variable=self.var_profile,
            value="bot_safe",
            command=self._on_profile_change
        ).pack(anchor="w", padx=5, pady=2)
        ...
        tk.Radiobutton(
            profiles_frame,
            text="🛠 Personalizado (establezca los valores a continuación)",
            variable=self.var_profile,
            value="custom",
            command=self._on_profile_change
        ...
        self.custom_frame = tk.LabelFrame(
            timing_frame, 
            text="Configuraciones personalizadas (solo este perfil)"
        )
        self.custom_frame.pack(fill="x", padx=10, pady=10)

        tk.Label(self.custom_frame, text="Presión mínima (ms):").grid(
        ...
        tk.Label(self.custom_frame, text="Pulsación máxima (ms):").grid(
        ...
        tk.Label(self.custom_frame, text="Intervalo mínimo (ms):").grid(
        ...
        tk.Label(self.custom_frame, text="Intervalo máximo (ms):").grid(
        ...
        tk.Label(self.custom_frame, text="Rango (+/- ms):").grid(
        ...
        if profile == "custom":
            try:
                ...
                messagebox.showerror(
                    "error", 
                    "Utilice sólo números en el modo Personalizado."
                )
                return
...
class iRacingControlApp:
    ...
    def __init__(self, root: tk.Tk):
        ...
        self.getting_started_text = (
            ...
            "1) Elige tu coche y pista.\n"
            "2) Confirmara tus dispositivos de entrada.\n"
            "3) Verifique los controles piloto.\n\n"
            "Si los selectores de coche/pista están atenuados, iRacing los gestionará "
            "automáticamente cuando te unes a una sesión. Únase a una sesión primero"
            "presets, or disable “Lock car/track selection” in the Options tab.\n\n"
            "Utilice el modo CONFIG cuando cambie los atajos y el modo RUN cuando conduzca."
        )
    ...
    def _apply_startup_preference(self, notify: bool = False) -> None:
        ...
        if notify:
            messagebox.showwarning(
                "Comience con Windows",
                "No se puede actualizar la entrada de inicio de Windows. "
                "Verifique los permisos o intente ejecutarlo como administrador."
            )
    ...
    def _drain_ui_queue(self):
        while True:
            ...
            try:
                ...
                print(f"[UI] error del controlador: {exc}")
    ...
    def _create_menu(self):
        ...
        options_menu.add_command(
            label="Ajustes de tiempo",
            command=self.open_timing_window
        )
        options_menu.add_command(
            label="Configuración de voz/audio",
            command=self.open_voice_audio_settings
        )
        options_menu.add_separator()
        options_menu.add_command(
            label="Mostrar/Ocultar superposición",
        ...
        options_menu.add_separator()
        options_menu.add_command(
            label="Restaurar valores predeterminados (eliminar configuración)",
    ...
    def _create_main_ui(self):
        ...
        self.main_tabs.add(main_tab, text="🏁 Principal")
        self.main_tabs.add(options_tab, text="⚙️ Opciones")
        ...
        presets_frame = tk.LabelFrame(
            steps_column,
            text="Paso 1: elige tu coche y rastrea"
        )
        ...
        tk.Label(selector_frame, text="Coche:").pack(side="izquierda")
        ...
        tk.Label(selector_frame, text="Pista:").pack(side="izquierda")
        ...
        devices_frame = tk.LabelFrame(
            steps_column,
            text="Paso 2: Confirmarar los dispositivos de entrada (joystick/volante)"
        )
        devices_frame.pack(fill="x", pady=(0, 8))

        self.check_safe = tk.Checkbutton(
            devices_frame,
            text="Modo solo teclado (requiere reinicio)",
        ...
        scan_frame = tk.LabelFrame(
            steps_column,
            text="Paso 3: Verifique los controles piloto"
        )
        scan_frame.pack(fill="x")

        self.btn_scan = tk.Button(
            scan_frame,
            text="Verificar controles para el auto seleccionado",
        ...
        tk.Label(
            scan_frame,
            text="Consejo: verifique después de cambiar de dispositivo o ajustes preestablecidos para mantener sincronizados los accesos directos.",
            fg="gray",
            font=("Arial", 9)
        ...
        options_notebook.add(general_tab, text="Configuraciones generales")
        ...
        general_left = tk.LabelFrame(
            general_container,
            text="Presets y dispositivos"
        )
        general_left.grid(row=0, column=0, sticky="nsew", pady=(0, 8))

        tk.Checkbutton(
            general_left,
            text="Guarde automáticamente las ediciones preestablecidas (atajos/macros)",
        ...
        tk.Checkbutton(
            general_left,
            text="Bloquear selección de coche/carril (gestionado automáticamente)",
        ...
        tk.Checkbutton(
            general_left,
            text="Detecta automáticamente coche/pista a través de iRacing",
        ...
        tk.Checkbutton(
            general_left,
            text="Comprueba automáticamente cuando cambia el coche/la pista",
        ...
        tk.Checkbutton(
            general_left,
            text="Mostrar ventana emergente de introducción al iniciar",
        ...
        tk.Button(
            stability_frame,
            text="Opciones de voz/audio",
            command=self.open_voice_audio_settings
        ).pack(fill="x", padx=8, pady=(0, 6))

        tk.Checkbutton(
            stability_frame,
            text="Reinicie antes de volver a verificar los controles (después de la primera verificación)",
        ...
        tk.Checkbutton(
            stability_frame,
            text="Reinicie y verifique automáticamente al ingresar a una sesión en ejecución",
        ...
        tk.Checkbutton(
            stability_frame,
            text="Mostrar ventana emergente de finalización de verificación",
        ...
        tk.Checkbutton(
            stability_frame,
            text="Sigue intentando alcanzar objetivos de atajos (sin tiempo de espera)",
        ...
        tk.Label(
            clear_frame,
            text="Acceso directo para borrar el objetivo (opcional):"
        ).pack(side="left")
        self.btn_clear_target_bind = tk.Button(
            clear_frame,
            text="Establecer acceso directo de limpieza",
            width=18,
        ...
        tk.Label(
            rescan_frame,
            text="Acceso directo de verificación manual (restablecer + verificar + cargar preajuste):"
        ).pack(side="left")
        self.btn_manual_rescan_bind = tk.Button(
            rescan_frame,
            text="Establecer acceso directo de verificación",
            width=18,
    ...
    def open_getting_started_window(self) -> None:
        ...
        self.getting_started_window.title("Primeros pasos")
    ...
    def _maybe_show_getting_started(self) -> None:
        """Ver the getting started popup once if enabled."""
    ...
    def _list_microphones(self) -> List[Tuple[int, str]]:
        devices: List[Tuple[int, str]] = [(-1, "Valor predeterminado del sistema")]
        if not HAS_SPEECH:
            return devices
        ...
        try:
            ...
            print(f"[Voz] No se pueden enumerar los micrófonos: {exc}")
            ...
            try:
                for idx in range(pa.get_device_count()):
                    ...
                    if info.get("maxOutputChannels", 0) > 0:
                        name = info.get("nombre", f"Salida {idx}")
                        devices.append((idx, name))
            finally:
                pa.terminate()
        except Exception as exc:  # noqa: PERF203
            print(f"[Audio] No se pueden enumerar los dispositivos de salida: {exc}")
    ...
    def _refresh_audio_device_lists(self):
        ...
        if self.mic_combo:
            ...
            current_label = self._device_label(
                ...
                dict(mic_devices).get(self.microphone_device.get(), "Valor predeterminado del sistema")
            )
        ...
        if self.audio_output_combo:
            ...
            current_output_label = self._device_label(
                ...
                dict(output_devices).get(self.audio_output_device.get(), "Valor predeterminado del sistema")
            )
    ...
    def open_voice_audio_settings(self):
        ...
        self.voice_window.title("Opciones de voz y audio")
        ...
        notebook.add(voice_tab, text="Voz/Audio")
    ...
    def close_voice_audio_window(self) -> None:
        """Cierre y limpie la ventana de opciones de voz/audio."""
    ...
    def _build_voice_audio_tab(self, parent: tk.Widget):
        ...
        engine_frame = tk.LabelFrame(parent, text="Mecanismo de reconocimiento")
        engine_frame.pack(fill="x", padx=2, pady=6)

        engine_row = tk.Frame(engine_frame)
        engine_row.pack(fill="x", padx=6, pady=2)

        ttk.Label(engine_row, text="Mecanismo de voz:").pack(side="izquierda", padx=4)
        ...
        self.btn_vosk_model = tk.Button(
            model_row,
            text="Seleccione el modelo Vosk...",
        ...
        self.btn_whisper_model = tk.Button(
            model_row,
            text="Seleccione el modelo Whisper...",
        ...
        device_frame = tk.LabelFrame(parent, text="Dispositivos de entrada/salida")
        device_frame.pack(fill="x", padx=2, pady=6)

        mic_row = tk.Frame(device_frame)
        mic_row.pack(fill="x", padx=6, pady=2)

        ttk.Label(mic_row, text="Micrófono:").pack(side="izquierda")
        ...
        ttk.Label(out_row, text="Salida de audio (TTS):").pack(side="izquierda")
        ...
        tuning_frame = tk.LabelFrame(
            parent,
            text="Ajuste de voz (precisión y velocidad)"
        )
        ...
        ttk.Label(tuning_row_1, text="Ruido(s) ambiental(es):").pack(side="izquierda")
        ttk.Spinbox(
            tuning_row_1,
            from_=0.0,
            to=3.0,
        ...
        ttk.Label(tuning_row_1, text="Duración máxima de la(s) sentencia(s):").pack(side="izquierda")
        ttk.Spinbox(
            tuning_row_1,
            from_=0.2,
            to=6.0,
        ...
        tk.Checkbutton(
            tuning_row_1,
            text="Potencia dinámica (automática)",
        ...
        ttk.Label(tuning_row_2, text="Tiempos de espera iniciales:").pack(side="izquierda")
        ttk.Spinbox(
            tuning_row_2,
            from_=0.0,
            to=5.0,
        ...
        ttk.Label(tuning_row_2, text="Tiempos de espera continuos:").pack(side="izquierda")
        ttk.Spinbox(
            tuning_row_2,
            from_=0.0,
            to=5.0,
        ...
        ttk.Label(tuning_row_2, text="Energía mínima:").pack(side="izquierda")
        ttk.Entry(
            tuning_row_2,
            width=8,
    ...
    def toggle_mode(self):
        ...
        if self.app_state == "RUNNING":
            ...
            self.btn_mode.config(
                text="Modo: CONFIG (Haga clic para guardar y ejecutar)",
                bg="orange"
            )
            ...
            self.btn_mode.config(text="Modo: CORRER", bg="#90ee90")
    ...
    def clear_all_targets(self):
        ...
        self.notify_overlay_status("Objetivos claros", "naranja")
    ...
    def trigger_safe_mode_update(self):
        """Activa la actualización del modo seguro con reinicio."""
        new_value = self.use_keyboard_only.get()
        if messagebox.askokcancel(
            "Reiniciar requerido",
            "Es necesario reiniciar para aplicar el modo de solo teclado. ¿Confirmarar?"
        ):
            self.save_config()
            restart_program()
        else:
    ...
    def open_device_manager(self):
        ...
        if self.use_keyboard_only.get():
            messagebox.showinfo(
                "Modo teclado",
                "Desactive el 'Modo solo teclado' para administrar dispositivos de joystick."
            )
            return
    ...
    def action_save_preset(self):
        ...
        if not car or not track:
            messagebox.showwarning("error", "Definir coche y pista.")
            return
    ...
    def _save_preset_for_pair(
        ...
        self.update_preset_ui()
        if show_message:
            messagebox.showinfo("Guardado", f"Preajuste guardado para {car} @ {track}")

    def load_specific_preset(self, car: str, track: str):
        """Cargar a specific car/track preset."""
        ...
        self.register_current_listeners()
        print(f"[Preestablecido] {car} / {track} cargado")

    def action_load_preset(self):
        """Cargar selected preset."""
        car = self.combo_car.get()
    ...
    def action_clear_preset(self):
        ...
        if car in self.saved_presets and track in self.saved_presets[car]:
            if not messagebox.askyesno(
                "Confirmarar", 
                f"¿Borrar {car} @ {track} preestablecido?"
            ):
                return
    ...
    def auto_preset_loop(self):
        ...
        try:
            ...
            print(f"[Detección automática] error: {e}")
        ...
        try:
            ...
            if isinstance(sessions, list):
                if session_num is not None and 0 <= session_num < len(sessions):
                    session_type = sessions[session_num].get("SessionTipo", "")
                if not session_type:
                    for entry in sessions:
                        session_type = entry.get("SessionTipo", "")
                        if session_type:
                            break
        except Exception:
            pass
    ...
    def _auto_scan_and_load_preset(self) -> None:
        """Verificación controls and then reload the current car/track preset."""
    ...
    def scan_driver_controls(
        ...
        *,
        ...
        """Verificación for dc* driver control variables in current car."""
        try:
            if self._scan_in_progress:
                if on_complete:
            ...
            print(f"[Verificar] error inesperado: {exc}")
            if not silent_if_unavailable:
                messagebox.showerror(
                    "Verificación",
                    "No se pudo verificar. Inténtalo de nuevo cuando estés en una sesión."
                )
            if on_complete:
                self.root.after(0, on_complete)
            self._scan_in_progress = False
    ...
    def _scan_driver_controls_worker(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                print(f"[Verificar] error al leer variables: {e}")
    ...
    def _finish_scan_driver_controls(
        ...
        if status == "unavailable":
            if not silent_if_unavailable:
                messagebox.showerror(
                    "error",
                    "Abre iRacing (o únete a una sesión)."
                )
            if on_complete:
                on_complete()
            return
        ...
        if status == "no_candidates":
            messagebox.showwarning(
                "Verificación",
                "El SDK aún no ha devuelto ninguna variable.\n"
                "Métete en el coche (Drive), ajusta los controles y vuelve a intentarlo."
            )
            if on_complete:
                on_complete()
            return
        ...
        if status == "no_vars":
            messagebox.showwarning(
                "Verificación",
                "No se encontró ninguna variable numérica 'dc*'.\n"
                "Es posible que el automóvil no tenga controles del conductor o que usted no esté en el modo Conducción."
            )
            if on_complete:
                on_complete()
            return
        ...
        if status != "ok":
            print(f"[Verificar] error inesperado: {result.get('error')}")
            if not silent_if_unavailable:
                messagebox.showerror(
                    "Verificación",
                    "No se pudo verificar. Inténtalo de nuevo cuando estés en una sesión."
    ...
    def notify_overlay_status(self, text: str, color: str):
        ...
        self.ui(
            self.root.after,
            2000,
            lambda: self.overlay.update_status_text("HUD listo", "blanco")
        )
    ...
    def _update_overlay_feedback(self):
        ...
        if state["abs_active"] >= cfg["abs_hold_s"]:
            self._push_overlay_alert(
                "ABS activo durante mucho tiempo: afloje el freno o reduzca el ABS.",
                "orange",
                cfg,
                now
            )
        ...
        if state["tc_active"] >= cfg["tc_hold_s"]:
            self._push_overlay_alert(
                "TC se activa constantemente: considere reducir TC o cambiar el mapa.",
                "orange",
                cfg,
                now
            )
        ...
        if state["spin_active"] >= cfg["wheelspin_hold_s"]:
            self._push_overlay_alert(
                "Deslizamiento detectado: aumentar el TC o modular el acelerador.",
                "orange",
                cfg,
                now
            )
        ...
        if state["lock_active"] >= cfg["lockup_hold_s"]:
            self._push_overlay_alert(
                "Bloqueo detectado: aumentar el ABS o aliviar la presión sobre el pedal.",
                "orange",
                cfg,
                now
            )
    ...
    def save_config(self):
        ...
        try:
            ...
            print(f"[GUARDAR] error al guardar la configuración: {e}")

    def load_config(self):
        """Cargar configuration from disk."""
    ...
    def _format_vosk_status(self) -> str:
        ...
        if engine != "vosk":
            if model_path:
                return f"Modelo Vosk seleccionado: {model_name or model_path}"
            return "Usando el reconocedor de voz de Windows"

        if not HAS_VOSK:
            if VOSK_IMPORT_ERROR:
                return f"Vosk no disponible: {VOSK_IMPORT_ERROR}"
            return "Vosk no instalado"

        if not model_path:
            return "Seleccione una carpeta de modelo Vosk"

        if voice_listener._vosk_error:
            return f"Error del modelo: {voice_listener._vosk_error}"

        if voice_listener.vosk_model is not None:
            name = model_name or model_path
            return f"Vosk listo: {name}"
    ...
    def _format_whisper_status_details(self) -> str:
        ...
        if not self.whisper_binary_path.get():
            if self.whisper_model_path.get():
                ...
                return f"Modelo Whisper seleccionado: {model_name}"
            return "Seleccione el ejecutable whisper.cpp"

        if not os.path.exists(self.whisper_binary_path.get()):
            return "Ejecutable no encontrado"

        if not self.whisper_model_path.get():
            exe_name = os.path.basename(self.whisper_binary_path.get())
            return f"whisper.cpp seleccionado: {exe_name} (elija un modelo)"

        if not os.path.exists(self.whisper_model_path.get()):
            return "Modelo no encontrado"

        if voice_listener._whisper_error:
            return f"Error de Whisper: {voice_listener._whisper_error}"

        model_name = os.path.basename(self.whisper_model_path.get())
        exe_name = os.path.basename(self.whisper_binary_path.get())
        return f"Whisper listo: {exe_name} | {model_name}"
    ...
    def choose_vosk_model(self):
        ...
        path = filedialog.askdirectory(title="Seleccione la carpeta del modelo Vosk")
        if not path:
            return
    ...
    def choose_whisper_binary(self):
        ...
        path = filedialog.askopenfilename(
            title="Seleccione el ejecutable whisper.cpp",
    ...
    def choose_whisper_model(self):
        ...
        path = filedialog.askopenfilename(
            title="Seleccione la plantilla Whisper (.bin/.gguf)",
            filetypes=[("Whisper models", "*.bin *.gguf"), ("All files", "*")]
        )
        if not path:
            return
    ...
    def open_voice_test_dialog(self):
        ...
        if not phrases_map:
            messagebox.showinfo(
                ...
                "Agregue frases a las pestañas para probar comandos de voz."
            )
            return
    ...
    def restore_defaults(self):
        ...
        if not messagebox.askyesno(
            ...
            "Esto eliminará su archivo de configuración y reiniciará la aplicación. ¿Continuar?"
        ):
            return
        ...
        try:
            ...
            messagebox.showerror(
                "error",
                f"No se pudo eliminar la configuración: {exc}"
            )
            return

        messagebox.showinfo(
            "Estándares restaurados",
            "Restablecimiento de configuración. La aplicación ahora se reiniciará."
        )
        restart_program()
...
def main():
    ...
    try:
        ...
        import traceback
        traceback.print_exc()
        input("Presione Entrar para cerrar...")
