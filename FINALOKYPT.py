...
def _normalize_timing_config(timing: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key in [
        ...
        except (TipoError, ValueError, KeyError):
...
class VoiceListener:
    ...
    def _ensure_whisper_ready(self) -> bool:
        ...
        if not os.path.exists(self.whisper_model_path):
            self._whisper_error = "Arquivo do modelo não encontrado"
            return False
    ...
    def capture_once(
        ...
        if not self.available or sr is None:
            return None, "Reconhecimento de voz indisponível."
    ...
    def __init__(
        ...
        info = tk.Label(
            ...
            text=(
                ...
                "Use o botão de teste para garantir que seu microfone e suas frases estejam"
                " funcionando."
            ),
            wraplength=400,
            justify="left"
...
class OverlayConfigTab(tk.Frame):
    ...
    def __init__(self, parent, app):
        ...
        feedback_frame = tk.LabelFrame(
            self.body, text="Limites de feedback de assistência (por carro)"
        )
        ...
        feedback_rows = [
            ...
            ("Patinagem (razão)", "wheelspin_slip"),
        ...
        variables_frame = tk.LabelFrame(
            self.body, 
            text="Variáveis para exibir (por carro)"
        )
...
class ControlTab(tk.Frame):
    ...
    def __init__(self, parent, controller: GenericController, label_name: str, app):
        ...
        tk.Label(
            presets_frame,
            text="RESET sempre retorna a um valor base (ex.: 0 ou 50). Adicione abaixo seus valores de macro preferidos.",
            fg="gray",
            font=("Arial", 8),
            wraplength=760,
            justify="left"
        ...
        tk.Label(
            presets_frame,
            text=(
                "Gatilho de voz opcional: digite a frase exata que você dirá para executar a "
                "macro. As Configurações de Voz/Áudio ficam em Opções → Configurações de "
                "Voz/Áudio."
            ),
            fg="gray",
            font=("Arial", 8),
    ...
    def run_bot_timing_probe(self):
        ...
        def _worker():
            ...
            if suggested is None:
                ...
                msg = (
                    f"Pulso mínimo estável detectado em ~{suggested} ms.\n"
                    "Aplique esse valor nos tempos BOT/personalizados para atualizações confiáveis."
                )
...
class ComboTab(tk.Frame):
    ...
    def __init__(
        ...
        tk.Label(
            body,
            text=(
                "Gatilho de voz opcional: digite a frase exata que você dirá para acionar este "
                "combo. As Configurações de Voz/Áudio ficam em Opções → Configurações de "
                "Voz/Áudio."
            ),
            fg="gray",
            font=("Arial", 8),
    ...
    def add_dynamic_row(
        ...
        bind_button = tk.Button(
            frame,
            text="RESET" if is_reset else "Definir atalho",
            width=15,
            fg="red" if is_reset else "black"
        )
...
class GlobalTimingWindow(tk.Toplevel):
    ...
    def __init__(self, parent, callback_save: Callable):
        super().__init__(parent)
        self.title("Ajustes de tempo (anti-detecção)")
        ...
        notebook.add(timing_frame, text="Tempo")
        ...
        self.custom_frame = tk.LabelFrame(
            timing_frame, 
            text="Configurações personalizadas (apenas este perfil)"
        )
        self.custom_frame.pack(fill="x", padx=10, pady=10)

        tk.Label(self.custom_frame, text="Pressão mín. (ms):").grid(
        ...
        tk.Label(self.custom_frame, text="Pressão máx. (ms):").grid(
...
class iRacingControlApp:
    ...
    def _apply_startup_preference(self, notify: bool = False) -> None:
        ...
        if notify:
            messagebox.showwarning(
                ...
                "Verifique permissões ou tente executar como Administrador."
            )
    ...
    def _create_main_ui(self):
        ...
        helper_text = (
            "Siga as etapas abaixo em ordem: 1) escolha seu carro e pista, "
            "2) confirme seus dispositivos de entrada e 3) escaneie os controles do piloto. "
            "Use o modo CONFIG ao alterar atalhos e o modo EM EXECUÇÃO ao dirigir."
        )
        tk.Label(
            helper_frame,
            text=helper_text,
        ...
        tk.Checkbutton(
            auto_frame,
            text="Autoescaneamento ao trocar carro/pista",
        ...
        tk.Checkbutton(
            stability_frame,
            text="Reiniciar antes de reescanear controles (após o primeiro scan)",
        ...
        tk.Checkbutton(
            stability_frame,
            text="Reiniciar e escanear automaticamente ao entrar em uma sessão de corrida",
        ...
        tk.Checkbutton(
            stability_frame,
            text="Mostrar pop-up de conclusão do scan",
        ...
        tk.Checkbutton(
            stability_frame,
            text="Continuar tentando alcançar alvos de atalho (sem timeout)",
        ...
        tk.Label(
            clear_frame,
            text="Atalho para limpar alvos (opcional):"
        ).pack(side="left")
        self.btn_clear_target_bind = tk.Button(
            clear_frame,
            text="Definir atalho de limpar",
            width=18,
        ...
        tk.Checkbutton(
            presets_frame,
            text="Salvar automaticamente edições de predefinições (atalhos/macros)",
        ...
        tk.Label(
            self.root,
            text="Dica: escaneie após trocar dispositivos ou predefinições para manter os atalhos sincronizados.",
            fg="gray",
            font=("Arial", 9)
    ...
    def _save_preset_for_pair(
        ...
        self.update_preset_ui()
        if show_message:
            messagebox.showinfo("Salvo", f"Predefinição salva para {car} @ {track}")
    ...
    def action_delete_preset(self):
        ...
        if car in self.saved_presets and track in self.saved_presets[car]:
            if not messagebox.askyesno(
                "Confirmar", 
                f"Excluir predefinição de {car} @ {track}?"
            ):
                return
    ...
    def _format_vosk_status(self) -> str:
        ...
        if not model_path:
            return "Selecione uma pasta de modelo do Vosk"
    ...
    def choose_whisper_model(self):
        ...
        path = filedialog.askopenfilename(
            title="Selecione o modelo Whisper (.bin/.gguf)",
            filetypes=[("Modelos Whisper", "*.bin *.gguf"), ("Todos os arquivos", "*")]
        )
        if not path:
            return
    ...
    def open_voice_test_dialog(self):
        ...
        if not phrases_map:
            messagebox.showinfo(
                ...
                "Adicione frases nas abas para testar os comandos de voz."
            )
            return
