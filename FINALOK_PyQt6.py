...
# ======================================================================
# PYQT6 TKINTER COMPATIBILITY LAYER
# ======================================================================


class _TkEventFilter(QtCore.QObject):
    def __init__(self, owner: "TkWidgetBase"):
        super().__init__()
        self.owner = owner

    def eventFilter(self, obj, event):  # noqa: N802
        return self.owner._handle_event(obj, event)


_GLOBAL_BINDINGS: Dict[str, List[Callable]] = {}


class _GlobalEventFilter(QtCore.QObject):
    def eventFilter(self, obj, event):  # noqa: N802
        if event.type() == QtCore.QEvent.Type.Wheel:
            delta = event.angleDelta().y()
            wrapper = type("WheelEvent", (), {"delta": delta})
            for callback in _GLOBAL_BINDINGS.get("<MouseWheel>", []):
                try:
                    callback(wrapper)
                except TypeError:
                    callback()
...
_GLOBAL_EVENT_FILTER = _GlobalEventFilter()


class TkVariable(QtCore.QObject):
    ...
    def trace_add(self, _mode, callback):
        ...
        self.changed.connect(_wrapped)


class StringVar(TkVariable):
    pass


class IntVar(TkVariable):
    pass


class BooleanVar(TkVariable):
    pass


class DoubleVar(TkVariable):
    pass


class TkWidgetBase(QtCore.QObject):
    ...
    def _register_child(self, child: "TkWidgetBase"):
    ...
    def bind_all(self, event_name, callback):
        _GLOBAL_BINDINGS.setdefault(event_name, []).append(callback)
        app = QtWidgets.QApplication.instance()
        if app and _GLOBAL_EVENT_FILTER not in app.children():
            app.installEventFilter(_GLOBAL_EVENT_FILTER)

    def unbind_all(self, event_name):
        _GLOBAL_BINDINGS.pop(event_name, None)

    def _emit_binding(self, event_name, event=None):
    ...
    def _handle_event(self, obj, event):
        ...
        if etype == QtCore.QEvent.Type.MouseButtonPress:
            self._emit_binding("<ButtonPress-1>", event)
            self._emit_binding("<Button-1>", event)
        elif etype == QtCore.QEvent.Type.MouseButtonRelease:
            self._emit_binding("<ButtonRelease-1>", event)
        elif etype == QtCore.QEvent.Type.MouseMove:
            if event.buttons() & QtCore.Qt.MouseButton.LeftButton:
                self._emit_binding("<B1-Motion>", event)
        elif etype == QtCore.QEvent.Type.Enter:
            self._emit_binding("<Enter>", event)
        elif etype == QtCore.QEvent.Type.Leave:
            self._emit_binding("<Leave>", event)
        elif etype in (QtCore.QEvent.Type.Resize, QtCore.QEvent.Type.Move):
            self._emit_binding("<Configure>", event)
        elif etype == QtCore.QEvent.Type.Wheel:
            self._emit_binding("<MouseWheel>", event)
        return False

    def pack(self, **kwargs):
        if self.master is not None:
            self.master._pack_child(self, **kwargs)

    def grid(self, **kwargs):
        if self.master is not None:
            self.master._grid_child(self, **kwargs)
...
class TkContainerMixin(TkWidgetBase):
    def _ensure_layout(self, kind, horizontal=False):
        if self._layout is not None and self._layout_kind == kind:
            return
    ...
    def _pack_child(self, child, **kwargs):
        side = kwargs.get("side", "top")
        fill = kwargs.get("fill")
        expand = kwargs.get("expand", False)

        horizontal = side in ("left", "right")
        self._ensure_layout("pack", horizontal=horizontal)

        if fill in ("x", "both"):
            child._widget.setSizePolicy(
                QtWidgets.QSizePolicy.Policy.Expanding,
                child._widget.sizePolicy().verticalPolicy(),
            )
        if fill in ("y", "both"):
            child._widget.setSizePolicy(
                child._widget.sizePolicy().horizontalPolicy(),
                QtWidgets.QSizePolicy.Policy.Expanding,
            )

        stretch = 1 if expand else 0
        self._layout.addWidget(child._widget, stretch=stretch)

    def _grid_child(self, child, **kwargs):
        row = kwargs.get("row", 0)
        column = kwargs.get("column", 0)
        rowspan = kwargs.get("rowspan", 1)
        colspan = kwargs.get("columnspan", 1)
        ...
        if "ns" in sticky or "sn" in sticky:
            child._widget.setSizePolicy(
                child._widget.sizePolicy().horizontalPolicy(),
                QtWidgets.QSizePolicy.Policy.Expanding,
            )
...
class Tk(TkContainerMixin):
    def __init__(self):
        ...
        self._widget = _QtWindow()
        self._window.setCentralWidget(self._widget)
        self._ensure_layout("pack")
        self._after_jobs: Dict[int, QtCore.QTimer] = {}
    ...
    def geometry(self, spec=None):
        ...
        if "x" in spec:
            size_part, *pos_parts = spec.split("+")
            width, height = size_part.split("x")
            x = int(pos_parts[0]) if len(pos_parts) > 0 else self._window.x()
            y = int(pos_parts[1]) if len(pos_parts) > 1 else self._window.y()
            self._window.setGeometry(x, y, int(width), int(height))
        elif spec.startswith("+"):
            parts = spec.split("+")
            if len(parts) >= 3:
                self._window.move(int(parts[1]), int(parts[2]))

    def config(self, **kwargs):
        menu = kwargs.get("menu")
        if menu is not None:
            self._window.setMenuBar(menu._menu_bar)

    def iconbitmap(self, path):
        self._window.setWindowIcon(QtGui.QIcon(path))

    def iconphoto(self, _default, image):
        if hasattr(image, "icon"):
            self._window.setWindowIcon(image.icon)
    ...
    def protocol(self, _name, callback):
        self._widget.close_callback = callback


class Toplevel(TkContainerMixin):
    def __init__(self, master=None):
    ...
    def geometry(self, spec=None):
        ...
        if "x" in spec:
            size_part, *pos_parts = spec.split("+")
            width, height = size_part.split("x")
            x = int(pos_parts[0]) if len(pos_parts) > 0 else self._widget.x()
            y = int(pos_parts[1]) if len(pos_parts) > 1 else self._widget.y()
            self._widget.setGeometry(x, y, int(width), int(height))
        elif spec.startswith("+"):
            parts = spec.split("+")
            if len(parts) >= 3:
                self._widget.move(int(parts[1]), int(parts[2]))

    def after(self, delay_ms, callback):
        timer = QtCore.QTimer(self._widget)
        timer.setSingleShot(True)
        timer.timeout.connect(callback)
        timer.start(delay_ms)
        self._after_jobs[id(timer)] = timer
    ...
    def overrideredirect(self, enabled):
        flags = self._widget.windowFlags()
        if enabled:
            self._widget.setWindowFlags(flags | QtCore.Qt.WindowType.FramelessWindowHint)
        else:
            self._widget.setWindowFlags(flags & ~QtCore.Qt.WindowType.FramelessWindowHint)
        self._widget.show()

    def wm_attributes(self, attr, value):
        if attr == "-topmost":
            flags = self._widget.windowFlags()
            if value:
            ...
            self._widget.show()
        if attr == "-alpha":
            self._widget.setWindowOpacity(float(value))


class Frame(TkContainerMixin):
    ...
    def configure(self, **kwargs):
        if "bg" in kwargs:
            self._widget.setStyleSheet(
                f"background-color: {kwargs['bg']};"
            )


class LabelFrame(TkContainerMixin):
    def __init__(self, master=None, text="", **kwargs):
        super().__init__(master=master)
        self._widget = QtWidgets.QGroupBox(text)
        self._ensure_layout("pack")
        self.configure(**kwargs)

    def configure(self, **kwargs):
        if "text" in kwargs:
            self._widget.setTitle(kwargs["text"])


class Label(TkWidgetBase):
    def __init__(self, master=None, text="", textvariable=None, **kwargs):
        super().__init__(master=master)
        self._widget = QtWidgets.QLabel(text)
        self._textvariable = textvariable
        if textvariable is not None:
            self._widget.setText(str(textvariable.get()))
            textvariable.changed.connect(
                lambda: self._widget.setText(str(textvariable.get()))
            )
        self.configure(**kwargs)

    def configure(self, **kwargs):
        if "text" in kwargs:
            self._widget.setText(kwargs["text"])
        styles = []
        if "fg" in kwargs:
            styles.append(f"color: {kwargs['fg']};")
        if "bg" in kwargs:
            styles.append(f"background-color: {kwargs['bg']};")
        if styles:
            self._widget.setStyleSheet(" ".join(styles))
        if "font" in kwargs:
            self._widget.setFont(_qt_font(kwargs["font"]))
...
class Button(TkWidgetBase):
    def __init__(self, master=None, text="", command=None, **kwargs):
        super().__init__(master=master)
        self._widget = QtWidgets.QPushButton(text)
        self._command = None
        if command:
            self._command = command
            self._widget.clicked.connect(command)
        self.configure(**kwargs)
    ...
    def configure(self, **kwargs):
        if "text" in kwargs:
            self._widget.setText(kwargs["text"])
        if "state" in kwargs:
            self._widget.setEnabled(kwargs["state"] != "disabled")
        if "command" in kwargs:
            if self._command:
                try:
                    self._widget.clicked.disconnect(self._command)
                except TypeError:
                    pass
            self._command = kwargs["command"]
            if self._command:
                self._widget.clicked.connect(self._command)


class Checkbutton(TkWidgetBase):
    def __init__(self, master=None, text="", variable=None, command=None, **kwargs):
        super().__init__(master=master)
        self._widget = QtWidgets.QCheckBox(text)
        self._variable = variable
        if variable is not None:
            self._widget.setChecked(bool(variable.get()))
            self._widget.stateChanged.connect(
                lambda state: variable.set(state == QtCore.Qt.CheckState.Checked)
            )
            variable.changed.connect(
                lambda: self._widget.setChecked(bool(variable.get()))
            )
        if command:
            self._widget.stateChanged.connect(lambda _state: command())
        self.configure(**kwargs)


class Radiobutton(TkWidgetBase):
    def __init__(self, master=None, text="", variable=None, value=None, **kwargs):
        super().__init__(master=master)
        self._widget = QtWidgets.QRadioButton(text)
        self._variable = variable
        self._value = value
        if variable is not None:
            self._widget.toggled.connect(self._on_toggled)
            variable.changed.connect(self._sync_from_var)
            self._sync_from_var()
        self.configure(**kwargs)

    def _on_toggled(self, checked):
        if checked and self._variable is not None:
            self._variable.set(self._value)

    def _sync_from_var(self):
        if self._variable is not None:
            self._widget.setChecked(self._variable.get() == self._value)


class Entry(TkWidgetBase):
    def __init__(self, master=None, textvariable=None, **kwargs):
        ...
        if textvariable is not None:
            self._widget.setText(str(textvariable.get()))
            self._widget.textChanged.connect(textvariable.set)
            textvariable.changed.connect(
                lambda: self._widget.setText(str(textvariable.get()))
            )
        self.configure(**kwargs)
    ...
    def insert(self, _index, text):
        current = self._widget.text()
        if _index in (0, "0"):
            self._widget.setText(str(text) + current)
        else:
            self._widget.setText(current + str(text))
    ...
    def configure(self, **kwargs):
        if "state" in kwargs:
            self._widget.setReadOnly(kwargs["state"] == "disabled")


class Spinbox(TkWidgetBase):
    def __init__(self, master=None, from_=0.0, to=100.0, increment=1.0,
                 textvariable=None, **kwargs):
        super().__init__(master=master)
        self._widget = QtWidgets.QDoubleSpinBox()
        self._widget.setRange(float(from_), float(to))
        self._widget.setSingleStep(float(increment))
        self._textvariable = textvariable
        if textvariable is not None:
            self._widget.setValue(float(textvariable.get() or 0))
            self._widget.valueChanged.connect(textvariable.set)
            textvariable.changed.connect(
                lambda: self._widget.setValue(float(textvariable.get() or 0))
            )
        self.configure(**kwargs)
    ...
    def configure(self, **kwargs):
        if "state" in kwargs:
            self._widget.setReadOnly(kwargs["state"] == "readonly")


class Scale(TkWidgetBase):
    def __init__(self, master=None, from_=0, to=100, orient="horizontal",
                 command=None, resolution=None, **kwargs):
        super().__init__(master=master)
        orientation = (
        ...
        self._widget = QtWidgets.QSlider(orientation)
        self._resolution = resolution
        if self._resolution:
            self._scale_factor = 1 / float(self._resolution)
        else:
            self._scale_factor = 1
            if isinstance(from_, float) or isinstance(to, float):
                self._scale_factor = 100
        self._widget.setRange(int(from_ * self._scale_factor), int(to * self._scale_factor))
        if command:
            self._widget.valueChanged.connect(
                lambda value: command(value / self._scale_factor)
            )
        self.configure(**kwargs)

    def get(self):
        return self._widget.value() / self._scale_factor

    def set(self, value):
        self._widget.setValue(int(float(value) * self._scale_factor))


class Scrollbar(TkWidgetBase):
    def __init__(self, master=None, orient="vertical", command=None, **kwargs):
        ...
        self._widget = QtWidgets.QScrollBar(orientation)
        self._command = command
        if command:
            self._widget.valueChanged.connect(lambda value: command("moveto", value))
        self.configure(**kwargs)
...
class Canvas(TkWidgetBase):
    def __init__(self, master=None, **kwargs):
        ...
        self._widget = self._scroll_area
        self._inner_widget: Optional[QtWidgets.QWidget] = None
        self._yscrollcommand = None
        self.configure(**kwargs)

    def create_window(self, _coords, window=None, anchor=None):
        if window is None:
            return None
        self._inner_widget = window._widget
        self._scroll_area.setWidget(window._widget)
        return window
    ...
    def configure(self, **kwargs):
        if "yscrollcommand" in kwargs:
            ...
            bar.valueChanged.connect(lambda value: self._yscrollcommand(value))

    def bbox(self, _tag):
        if self._inner_widget is None:
            return (0, 0, 0, 0)
        rect = self._inner_widget.geometry()
        return (rect.x(), rect.y(), rect.width(), rect.height())

    def yview(self, _action=None, value=None):
        if value is not None:
            self._scroll_area.verticalScrollBar().setValue(int(value))

    def yview_scroll(self, units, _what):
        bar = self._scroll_area.verticalScrollBar()
        bar.setValue(bar.value() + int(units) * bar.singleStep())


class Menu(TkWidgetBase):
    def __init__(self, master=None, tearoff=0, **kwargs):
        super().__init__(master=master)
        self._menu_bar = None
        self._menu = None
        if master is None or isinstance(master, Tk):
    ...
    def add_command(self, label="", command=None):
        action = QtGui.QAction(label, self._menu or self._menu_bar)
        if command:
            action.triggered.connect(command)
        if self._menu is not None:
            self._menu.addAction(action)
        elif self._menu_bar is not None:
    ...
    def add_cascade(self, label="", menu=None):
        if self._menu_bar is not None and menu is not None:
            menu._menu.setTitle(label)
            self._menu_bar.addMenu(menu._menu)


class PhotoImage:
    def __init__(self, file):
        self.icon = QtGui.QIcon(file)


class ComboBox(TkWidgetBase):
    def __init__(self, master=None, values=None, state="normal", **kwargs):
        super().__init__(master=master)
        self._widget = QtWidgets.QComboBox()
        if values:
            self._widget.addItems(list(values))
        if state == "readonly":
    ...
    def bind(self, event_name, callback):
        if event_name == "<<ComboboxSelected>>":
            self._widget.currentIndexChanged.connect(lambda _idx: callback())
        else:
            super().bind(event_name, callback)

    def configure(self, **kwargs):
        if "values" in kwargs:
            self._widget.clear()
            self._widget.addItems(list(kwargs["values"]))
...
class Notebook(TkContainerMixin):
    def __init__(self, master=None, **kwargs):
        super().__init__(master=master)
        self._widget = QtWidgets.QTabWidget()
        self._tabs: Dict[int, TkWidgetBase] = {}
        self.configure(**kwargs)

    def add(self, child, text=""):
        self._tabs[id(child)] = child
        self._widget.addTab(child._widget, text)

    def select(self, child):
        index = self._widget.indexOf(child._widget)
        if index >= 0:
            self._widget.setCurrentIndex(index)
    ...
    def forget(self, tab_id):
        child = self._tabs.pop(tab_id, None)
        if child:
            index = self._widget.indexOf(child._widget)
            if index >= 0:
...
def _qt_font(font_spec):
    if isinstance(font_spec, QtGui.QFont):
        return font_spec
    if isinstance(font_spec, tuple):
        family = font_spec[0]
        size = int(font_spec[1]) if len(font_spec) > 1 else 10
        weight = QtGui.QFont.Weight.Bold if "bold" in font_spec else QtGui.QFont.Weight.Normal
...
class _ColorChooser:
    @staticmethod
    def askcolor(title=""):
        color = QtWidgets.QColorDialog.getColor(title=title)
        if not color.isValid():
            return (None, None)
        return (color.getRgb(), color.name())


class _FileDialog:
    @staticmethod
    def askopenfilename(**kwargs):
        return QtWidgets.QFileDialog.getOpenFileName(**kwargs)[0]
    ...
    def asksaveasfilename(**kwargs):
        return QtWidgets.QFileDialog.getSaveFileName(**kwargs)[0]
