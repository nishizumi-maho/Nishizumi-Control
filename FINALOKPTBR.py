    ...
    def _run(self):
        while not self._stop_event.wait(self.interval_s):
            ...
            try:
                ...
                print(f"[Watchdog:{self.name}] Falha na recuperação: {exc}")
...
try:
    ...
    HAS_PYGAME = False
    print("Aviso: 'pygame' não instalado. Suporte a joystick desativado.")
...
try:
    ...
    HAS_TTS = False
    print("Aviso: 'pyttsx3' não instalado. TTS desativado.")
...
try:
    ...
    pyaudio = None
    HAS_PYAUDIO = False
    print("Aviso: 'pyaudio' não instalado. Seleção de dispositivo de áudio limitada.")
...
if _speech_spec is not None:
    ...
    sr = None
    HAS_SPEECH = False
    print("Aviso: 'speech_recognition' não instalado. Gatilhos de voz desativados.")
...
try:
    ...
    print(f"Aviso: 'vosk' indisponível: {exc}")
...
def apply_app_icon(root: tk.Tk) -> None:
    ...
    for icon_name in ICON_CANDIDATES:
        ...
        try:
            ...
            print(f"[Ícone] Falha ao carregar {icon_path}: {exc}")
...
# Temporização profiles for input simulation (click-click timing)
GLOBAL_TIMING = {
...
def restart_program():
    ...
    try:
        ...
        print(f"[Reinício] Falha ao iniciar novo processo: {exc}")
...
def set_startup_entry(enabled: bool) -> bool:
    ...
    try:
        ...
        print(f"[Inicialização] Falha ao atualizar entrada: {exc}")
        return False
...
def mark_pending_scan(*, silent: bool = False) -> None:
    ...
    try:
        ...
        print(f"[PendingScan] Falha ao persistir marcador: {exc}")
...
def consume_pending_scan() -> Tuple[bool, bool]:
    ...
    try:
        ...
        print(f"[PendingScan] Falha ao ler marcador: {exc}")
    finally:
        try:
            os.remove(PENDING_SCAN_FILE)
        except Exception as exc:
            print(f"[PendingScan] Falha ao limpar marcador: {exc}")
...
if IS_WINDOWS:
    ...
    SendInput = None
    print("Aviso: APIs SendInput do Windows indisponíveis; injeção de entrada desativada.")
...
def press_key(scan_code: int):
    ...
    if SendInput is None:
        raise OSError("As APIs SendInput estão disponíveis apenas em plataformas Windows.")
    ...
    with _TTS_LOCK:
        if _TTS_ENGINE is None:
            try:
                ...
                print(f"[TTS] Falha ao inicializar o mecanismo: {exc}")
                _TTS_ENGINE = None
...
def _play_wave_file(path: str, output_device: Optional[int]) -> bool:
    ...
    try:
        ...
        print(f"[TTS] Falha na reprodução de áudio: {exc}")
        return False
...
def _tts_worker():
    ...
    while True:
        ...
        try:
            ...
            print(f"[TTS] Falha na reprodução: {exc}")
        finally:
            _TTS_QUEUE.task_done()
...
class InputManager:
    ...
    def __init__(self):
        ...
        if HAS_PYGAME:
            try:
                ...
                print(f"[InputManager] Erro ao iniciar o pygame: {e}")
        ...
        if self.safe_mode:
            ...
            if HAS_PYGAME:
                try:
                    ...
                    print(f"[InputManager] Erro ao reativar pygame: {e}")
        ...
        try:
            ...
            for i in range(count):
                try:
                    ...
                    devices.append((i, f"Dispositivo {i} (Erro)"))
                    
            return devices
        except Exception as e:
            print(f"[InputManager] Erro ao obter dispositivos: {e}")
            return []
            ...
            for i in range(pygame.joystick.get_count()):
                ...
                if j.get_name() in self.allowed_devices:
                    try:
                        ...
                        print(f"[InputManager] Conectado: {j.get_name()}")
                    except Exception:
                        pass
        except Exception:
            pass
    ...
    def _restart_input_loop(self):
        ...
        print("[InputManager][Watchdog] Loop de entrada sem resposta, reiniciando...")
...
class VoiceListener:
    ...
    def _recover_listener(self):
        ...
        print("[Voz][Watchdog] Thread do listener sem resposta, reiniciando...")
        self.start()
    ...
    def _init_vosk_model(self, model_path: str):
        ...
        try:
            ...
            print(f"[Voz][Vosk] Falha ao carregar o modelo: {exc}")

    def _ensure_whisper_ready(self) -> bool:
        """Validate whisper.cpp prerequisites."""

        if not self.whisper_binary_path:
            self._whisper_error = "Selecione o executável do whisper.cpp"
            return False

        if not os.path.exists(self.whisper_binary_path):
            self._whisper_error = "Executável do whisper.cpp não encontrado"
            return False

        if not os.access(self.whisper_binary_path, os.X_OK):
            self._whisper_error = "whisper.cpp não é executável"
            return False

        if not self.whisper_model_path:
            self._whisper_error = "Selecione um modelo ggml/gguf (.bin/.gguf)"
            return False

        if not os.path.exists(self.whisper_model_path):
            self._whisper_error = "Arquivo de modelo não encontrado"
            return False
    ...
    def _recognize_text(self, audio, recognizer=None) -> Optional[str]:
        ...
        if self.engine == "vosk" and HAS_VOSK and self.vosk_model:
            try:
                ...
                print(f"[Voz][Vosk] Erro de reconhecimento: {exc}")
    ...
    def _transcribe_with_whisper(self, audio) -> Optional[str]:
        ...
        try:
            ...
            if result.returncode != 0:
                self._whisper_error = result.stderr.strip() or "whisper.cpp falhou"
                return None
            ...
            self._whisper_error = "Nenhum texto retornado pelo whisper.cpp"
            return None
        except Exception as exc:
            self._whisper_error = str(exc)
            print(f"[Voz][Whisper] Erro de reconhecimento: {exc}")
            return None
        finally:
            if tmp_path:
                try:
    ...
    def _listen_loop(self):
        ...
        try:
            ...
            print(f"[Voz] Listener interrompido: {exc}")
    ...
    def capture_once(
        ...
        if not self.available or sr is None:
            return None, "Reconhecimento de voz não disponível."
...
class VoiceTestDialog(tk.Toplevel):
    ...
    def __init__(
        ...
        info = tk.Label(
            ...
            text=(
                ...
                "Use o botão de teste para garantir que seu microfone e frases estejam"
                " funcionando."
            ),
            wraplength=400,
            justify="left"
        ...
        self.heard_var = tk.StringVar(value="(ainda nada)")
    ...
    def start_listen(self):
        ...
        self.btn_listen.config(state="disabled", text="Ouvindo...")
        self.status_var.set("Fale o comando configurado agora...")
        self.heard_var.set("(ouvindo)")
    ...
    def _listen_worker(self):
        ...
        def finalize():
            ...
            if triggered:
                self.status_var.set("Macro acionada com sucesso!")
            else:
                self.status_var.set("Frase reconhecida, mas nenhuma macro está vinculada.")
    ...
    def run_manual_phrase(self):
        """Gatilho macro manually from text input."""
        phrase = self.entry_manual.get().strip().lower()
        if not phrase:
            self.status_var.set("Digite uma frase para testar.")
            return

        if self._trigger_phrase(phrase):
            self.status_var.set("Macro executada manualmente.")
        else:
            self.status_var.set("Nenhuma macro está vinculada a essa frase.")
...
class DeviceSelector(tk.Toplevel):
    ...
    def __init__(self, parent, current_allowed: List[str], callback: Callable[[List[str]], None]):
        super().__init__(parent)
        self.title("Gerenciar dispositivos USB")
        self.geometry("450x400")
        self.callback = callback

        tk.Label(
            self,
            text="Selecione quais dispositivos o aplicativo pode usar",
            font=("Arial", 10, "bold"),
            pady=10
        ).pack()
        ...
        tk.Label(
            self,
            text="Marque/desmarque para permitir/proibir o uso do dispositivo",
            fg="gray"
        ).pack()
...
class OverlayConfigTab(tk.Frame):
    ...
    def __init__(self, parent, app):
        ...
        tk.Label(
            self.body,
            text="Configuração do HUD / Overlay",
            font=("Arial", 11, "bold")
        ).pack(anchor="w", pady=(5, 5))

        # Global appearance settings
        appearance_frame = tk.LabelFrame(self.body, text="Aparência global do HUD")
        ...
        tk.Label(appearance_frame, text="Tamanho da fonte:").grid(
        ...
        tk.Label(appearance_frame, text="Opacidade:").grid(
        ...
        feedback_frame = tk.LabelFrame(
            self.body, text="Limiares de feedback de assistências (por carro)"
        )
        feedback_frame.pack(fill="x", padx=5, pady=5)

        tk.Checkbutton(
            feedback_frame,
            text="Mostrar dicas de ABS / TC / derrapagem no HUD",
        ...
        feedback_rows = [
            ...
            ("Derrapagem de roda (razão)", "wheelspin_slip"),
            ("Duração da patinagem (s)", "wheelspin_hold_s"),
            ("Derrapagem de travamento (valor negativo)", "lockup_slip"),
            ("Duração do travamento (s)", "lockup_hold_s"),
            ("Intervalo entre alertas (s)", "cooldown_s"),
        ]
        ...
        variables_frame = tk.LabelFrame(
            self.body, 
            text="Variáveis a exibir (por carro)"
        )
        ...
        tk.Label(header, text="Exibir", width=8, anchor="w").pack(
            side="left", padx=2
        )
        tk.Label(header, text="Nome interno", width=25, anchor="w").pack(
            side="left", padx=2
        )
        tk.Label(header, text="Rótulo do HUD", width=20, anchor="w").pack(
            side="left", padx=2
        )
    ...
    def pick_background_color(self):
        ...
        color = colorchooser.askcolor(title="Cor de fundo")[1]
        if color:
    ...
    def pick_text_color(self):
        ...
        color = colorchooser.askcolor(title="Cor do texto")[1]
        if color:
...
class GenericController:
    ...
    def _resolve_target(self, target: float) -> float:
        ...
        if abs(aligned - target) >= 0.0005:
            if self.update_status:
                self.update_status(f"Arredondado para {aligned:.3f}", "orange")
            if self.app:
                short_name = self.var_name.replace("dc", "")
                self.app.notify_overlay_status(
                    f"{short_name}: usando {aligned:.3f} (mais próximo)",
                    "orange"
                )
    ...
    def _run_target_loop(self):
        ...
        if not self.key_increase or not self.key_decrease:
            if self.update_status:
                self.update_status("Nenhuma tecla configurada", "red")
            if self.app:
                self.app.notify_overlay_status(
                    f"{self.var_name.replace('dc', '')}: Sem teclas",
                    "red"
                )
            return
        ...
        try:
            while True:
                ...
                if pending_target != active_request:
                    ...
                    if self.update_status:
                        self.update_status("Ajustando...", "orange")
                    if self.app:
                        self.app.notify_overlay_status(
                            f"Ajustando {short_name} -> {active_target}",
                            "orange"
                        )
            ...
            print(f"[GenericController] Exceção: {exc}")
        finally:
            if success:
                ...
                if self.update_status:
                    self.update_status("Cancelado", "red")
                if self.app:
                    self.app.notify_overlay_status(
...
class ControlTab(tk.Frame):
    ...
    def __init__(self, parent, controller: GenericController, label_name: str, app):
        ...
        tk.Button(
            keys_frame,
            text="Testar tempo mínimo personalizado",
        ...
        # Status label
        self.lbl_status = tk.Label(body, text="Ocioso", fg="gray")
        self.lbl_status.pack()

        # Presets/Macros
        presets_frame = tk.LabelFrame(
            body, 
            text="Predefinições / Macros", 
            padx=5, 
            pady=5
        )
        ...
        tk.Label(
            presets_frame,
            text="RESETAR sempre retorna para um valor base (ex.: 0 ou 50). Adicione abaixo os valores de macro que você mais usa.",
            fg="gray",
            font=("Arial", 8),
            wraplength=760,
            justify="left"
        ...
        tk.Label(
            presets_frame,
            text=(
                "Gatilho de voz opcional: digite a frase exata que você dirá para executar a macro. "
                "As Configurações de Voz/Áudio ficam em Opções → Configurações de Voz/Áudio."
            ),
            fg="gray",
            font=("Arial", 8),
            wraplength=760,
    ...
    def run_bot_timing_probe(self):
        ...
        def _worker():
            try:
                ...
                self.after(
                    0,
                    lambda msg=error_msg: messagebox.showerror("Teclas ausentes", msg)
                )
                return
            ...
            if suggested is None:
                self.after(
                    ...
                    lambda: messagebox.showwarning(
                        ...
                        "Nenhuma temporização entre 1 e 120 ms atualizou a telemetria de forma confiável."
                    )
                )
            else:
                msg = (
                ...
                self.after(0, lambda: messagebox.showinfo("Resultado da verificação", msg))
    ...
    def bind_game_key(self, direction: str):
        ...
        btn.config(text="PRESSIONE A TECLA...", bg="yellow")
    ...
    def add_preset_row(
        ...
        label_text = "RESETAR" if is_reset else "Macro"
        tk.Label(
            frame,
            text=label_text,
            width=6,
    ...
    def monitor_loop(self):
        ...
        try:
            self.lbl_monitor.config(text=f"Atual: {text}")
        except Exception:
            pass
    ...
    def set_config(self, config: Dict[str, Any]):
        ...
        self.btn_increase.config(text=config.get("key_increase_text", "Definir aumento (+)"))
        self.btn_decrease.config(text=config.get("key_decrease_text", "Definir diminuição (-)"))
...
# Due to length, I'll create a third artifact for ComboTab, GlobalTemporizaçãoWindow, 
...
class ComboTab(tk.Frame):
    ...
    def __init__(
        ...
        tk.Label(
            body,
            text="⚡ Ajustes de combo (um gatilho -> várias variáveis)",
            fg="orange",
            font=("Arial", 10, "bold")
        ).pack(pady=5)
        ...
        tk.Label(
            body,
            text=(
                "Gatilho de voz opcional: digite a frase exata que você dirá para acionar este combo. "
                "As Configurações de Voz/Áudio ficam em Opções → Configurações de Voz/Áudio."
            ),
            fg="gray",
            font=("Arial", 8),
...
# ======================================================================
class GlobalTemporizaçãoWindow(tk.Toplevel):
    """
    ...
    def __init__(self, parent, callback_save: Callable):
        super().__init__(parent)
        self.title("Ajustes de temporização (anti-detecção)")
        ...
        notebook.add(timing_frame, text="Temporização")

        # Profile selection
        profiles_frame = tk.LabelFrame(timing_frame, text="Perfis de comportamento")
        ...
        tk.Radiobutton(
            profiles_frame,
            text="🤖 BOT (experimental, atraso quase zero)",
        ...
        tk.Radiobutton(
            profiles_frame,
            text="🤖 BOT Estável (rápido, mais confiável)",
            variable=self.var_profile,
            value="bot_safe",
            command=self._on_profile_change
        ).pack(anchor="w", padx=5, pady=2)
        ...
        tk.Radiobutton(
            profiles_frame,
            text="😈 Agressivo (rápido, robótico)",
            variable=self.var_profile,
            value="aggressive",
            command=self._on_profile_change
        ).pack(anchor="w", padx=5, pady=2)
        ...
        tk.Radiobutton(
            profiles_frame,
            text="🛠 Personalizado (defina os valores abaixo)",
            variable=self.var_profile,
            value="custom",
            command=self._on_profile_change
        ...
        self.custom_frame = tk.LabelFrame(
            timing_frame, 
            text="Configurações personalizadas (somente este perfil)"
        )
        self.custom_frame.pack(fill="x", padx=10, pady=10)

        tk.Label(self.custom_frame, text="Pressionamento mín (ms):").grid(
        ...
        tk.Label(self.custom_frame, text="Pressionamento máx (ms):").grid(
        ...
        tk.Label(self.custom_frame, text="Intervalo mín (ms):").grid(
        ...
        tk.Label(self.custom_frame, text="Intervalo máx (ms):").grid(
        ...
        tk.Label(self.custom_frame, text="Faixa (+/- ms):").grid(
        ...
        if profile == "custom":
            try:
                ...
                messagebox.showerror(
                    "Erro", 
                    "Use apenas números no modo Personalizado."
                )
                return
...
class iRacingControlApp:
    ...
    def __init__(self, root: tk.Tk):
        ...
        self.getting_started_text = (
            ...
            "1) Escolha seu carro e pista.\n"
            "2) Confirme seus dispositivos de entrada.\n"
            "3) Verifique os controles do piloto.\n\n"
            "Se os seletores de carro/pista estiverem esmaecidos, o iRacing irá gerenciá-los "
            "automaticamente quando você entrar em uma sessão. Entre em uma sessão primeiro "
            "para trabalhar com presets ou desative “Bloquear seleção de carro/pista” na aba Opções.\n\n"
            "Use o modo CONFIG ao alterar atalhos e o modo EXECUÇÃO ao dirigir."
        )
    ...
    def _apply_startup_preference(self, notify: bool = False) -> None:
        ...
        if notify:
            messagebox.showwarning(
                ...
                "Não foi possível atualizar a entrada de inicialização do Windows. "
                "Verifique as permissões ou tente executar como Administrador."
            )
    ...
    def _drain_ui_queue(self):
        while True:
            ...
            try:
                ...
                print(f"[UI] Erro no manipulador: {exc}")
    ...
    def _create_menu(self):
        ...
        options_menu.add_command(
            label="Ajustes de temporização",
            command=self.open_timing_window
        )
        options_menu.add_command(
            label="Configurações de voz/áudio",
        ...
        options_menu.add_separator()
        options_menu.add_command(
            label="Restaurar padrões (excluir configuração)",
    ...
    def _create_main_ui(self):
        ...
        self.main_tabs.add(options_tab, text="⚙️ Opções")
        ...
        presets_frame = tk.LabelFrame(
            steps_column,
            text="Etapa 1: Escolha seu carro e pista"
        )
        ...
        tk.Label(selector_frame, text="Carro:").pack(side="left")
        ...
        tk.Label(selector_frame, text="Pista:").pack(side="left")
        ...
        devices_frame = tk.LabelFrame(
            steps_column,
            text="Etapa 2: Confirme os dispositivos de entrada (joystick/volante)"
        )
        devices_frame.pack(fill="x", pady=(0, 8))

        self.check_safe = tk.Checkbutton(
            devices_frame,
            text="Modo somente teclado (requer reinicialização)",
        ...
        scan_frame = tk.LabelFrame(
            steps_column,
            text="Etapa 3: Verificar controles do piloto"
        )
        scan_frame.pack(fill="x")

        self.btn_scan = tk.Button(
            scan_frame,
            text="Verificar controles para o carro selecionado",
        ...
        tk.Label(
            scan_frame,
            text="Dica: verifique após trocar dispositivos ou predefinições para manter os atalhos sincronizados.",
            fg="gray",
            font=("Arial", 9)
        ...
        options_notebook.add(general_tab, text="Configurações gerais")
        ...
        general_left = tk.LabelFrame(
            general_container,
            text="Predefinições e dispositivos"
        )
        general_left.grid(row=0, column=0, sticky="nsew", pady=(0, 8))

        tk.Checkbutton(
            general_left,
            text="Salvar automaticamente edições das predefinições (atalhos/macros)",
        ...
        tk.Checkbutton(
            general_left,
            text="Bloquear seleção de carro/pista (gerenciado automaticamente)",
        ...
        tk.Checkbutton(
            general_left,
            text="Detectar carro/pista automaticamente via iRacing",
        ...
        tk.Checkbutton(
            general_left,
            text="Verificar automaticamente quando carro/pista mudar",
        ...
        tk.Checkbutton(
            general_left,
            text="Mostrar o pop-up de primeiros passos ao iniciar",
        ...
        stability_frame = tk.LabelFrame(
            general_container,
            text="Automação e atalhos"
        )
        stability_frame.grid(row=1, column=0, sticky="nsew")

        tk.Button(
            stability_frame,
            text="Ajustes de temporização",
            command=self.open_timing_window
        ...
        tk.Checkbutton(
            stability_frame,
            text="Reiniciar antes de verificar novamente os controles (após a primeira verificação)",
        ...
        tk.Checkbutton(
            stability_frame,
            text="Reiniciar e verificar automaticamente ao entrar em uma sessão de corrida",
        ...
        tk.Checkbutton(
            stability_frame,
            text="Mostrar pop-up de conclusão da verificação",
        ...
        tk.Checkbutton(
            stability_frame,
            text="Desativar comandos quando IsOnTrackCar estiver falso",
        ...
        tk.Label(
            clear_frame,
            text="Atalho para limpar alvo (opcional):"
        ).pack(side="left")
        self.btn_clear_target_bind = tk.Button(
            clear_frame,
            text="Definir atalho de limpeza",
            width=18,
        ...
        tk.Label(
            rescan_frame,
            text="Atalho de verificação manual (reiniciar + verificar + carregar preset):"
        ).pack(side="left")
        self.btn_manual_rescan_bind = tk.Button(
            rescan_frame,
            text="Definir atalho de verificação",
            width=18,
    ...
    def _list_microphones(self) -> List[Tuple[int, str]]:
        ...
        try:
            ...
            print(f"[Voz] Não foi possível listar microfones: {exc}")
            ...
            try:
                for idx in range(pa.get_device_count()):
                    ...
                    if info.get("maxOutputChannels", 0) > 0:
                        name = info.get("name", f"Saída {idx}")
                        devices.append((idx, name))
            finally:
                pa.terminate()
        except Exception as exc:  # noqa: PERF203
            print(f"[Áudio] Não foi possível listar dispositivos de saída: {exc}")
    ...
    def close_voice_audio_window(self) -> None:
        """Fechar and clean up the voice/audio options window."""
    ...
    def _build_voice_audio_tab(self, parent: tk.Widget):
        ...
        if not HAS_SPEECH:
            tk.Label(
                toggles_frame,
                text="(Instale 'speech_recognition' para voz)",
                fg="gray",
                font=("Arial", 8)
            ).pack(side="left", padx=4)

        engine_frame = tk.LabelFrame(parent, text="Mecanismo de reconhecimento")
        engine_frame.pack(fill="x", padx=2, pady=6)

        engine_row = tk.Frame(engine_frame)
        engine_row.pack(fill="x", padx=6, pady=2)

        ttk.Label(engine_row, text="Mecanismo de voz:").pack(side="left", padx=4)
        ...
        self.btn_whisper_model = tk.Button(
            model_row,
            text="Selecionar modelo Whisper...",
        ...
        tuning_frame = tk.LabelFrame(
            parent,
            text="Ajuste de voz (precisão e velocidade)"
        )
        ...
        ttk.Label(tuning_row_1, text="Duração máxima da frase (s):").pack(side="left")
        ttk.Spinbox(
            tuning_row_1,
            from_=0.2,
            to=6.0,
        ...
        tk.Checkbutton(
            tuning_row_1,
            text="Energia dinâmica (automático)",
        ...
        ttk.Label(tuning_row_2, text="Tempo limite inicial (s):").pack(side="left")
        ttk.Spinbox(
            tuning_row_2,
            from_=0.0,
            to=5.0,
        ...
        ttk.Label(tuning_row_2, text="Tempo limite contínuo (s):").pack(side="left")
        ttk.Spinbox(
            tuning_row_2,
            from_=0.0,
            to=5.0,
    ...
    def clear_all_targets(self):
        ...
        self.notify_overlay_status("Alvos limpos", "orange")
    ...
    def trigger_safe_mode_update(self):
        """Gatilho safe mode update with restart."""
        new_value = self.use_keyboard_only.get()
        if messagebox.askokcancel(
            "Reinicialização necessária",
            "É necessário reiniciar para aplicar o modo somente teclado. Confirmar?"
        ):
            self.save_config()
            restart_program()
        else:
    ...
    def open_device_manager(self):
        ...
        if self.use_keyboard_only.get():
            messagebox.showinfo(
                "Modo teclado",
                "Desative o 'Modo somente teclado' para gerenciar dispositivos de joystick."
            )
            return
    ...
    def action_save_preset(self):
        ...
        if not car or not track:
            messagebox.showwarning("Erro", "Defina carro e pista.")
            return
    ...
    def _save_preset_for_pair(
        ...
        self.update_preset_ui()
        if show_message:
            messagebox.showinfo("Salvo", f"Preset salvo para {car} @ {track}")
    ...
    def load_specific_preset(self, car: str, track: str):
        ...
        self.register_current_listeners()
        print(f"[Preset] Carregado {car} / {track}")
    ...
    def action_export_preset_values(self) -> None:
        ...
        if not self.tabs and not self.combo_tab:
            messagebox.showwarning("Exportar", "Nenhum preset para exportar.")
            return

        path = filedialog.asksaveasfilename(
            title="Exportar valores do preset",
            defaultextension=".json",
            filetypes=[("Valores do Preset", "*.json"), ("Todos os arquivos", "*.*")]
        )
        if not path:
            return
    ...
    def action_import_preset_values(self) -> None:
        ...
        try:
            ...
            messagebox.showerror(
                "Falha ao importar",
                f"Não foi possível importar o preset:\n{exc}"
            )
            return
        ...
        if applied_tabs == 0:
            messagebox.showwarning(
                "Importar",
                "Nenhum preset compatível encontrado para os controles atuais."
            )
            return
        ...
        messagebox.showinfo(
            "Importado",
            "Valores do preset importados. As teclas foram preservadas."
        )
    ...
    def scan_driver_controls(
        ...
        *,
        ...
        try:
            ...
            print(f"[Verificação] Erro inesperado: {exc}")
            if not silent_if_unavailable:
                messagebox.showerror(
                    "Verificação",
                    "Falha ao verificar. Tente novamente quando estiver em uma sessão."
                )
            if on_complete:
                self.root.after(0, on_complete)
            self._scan_in_progress = False
    ...
    def _finish_scan_driver_controls(
        ...
        if status == "unavailable":
            if not silent_if_unavailable:
                messagebox.showerror(
                    "Erro",
                    "Abra o iRacing (ou entre em uma sessão)."
                )
            if on_complete:
                on_complete()
            return
        ...
        if status == "no_candidates":
            messagebox.showwarning(
                "Verificação",
                "O SDK ainda não retornou nenhuma variável.\n"
                "Entre no carro (Drive), ajuste os controles e tente novamente."
            )
            if on_complete:
                on_complete()
            return
        ...
        if status == "no_vars":
            messagebox.showwarning(
                "Verificação",
                "Nenhuma variável numérica 'dc*' encontrada.\n"
                "O carro pode não ter controles do piloto ou você não está no modo Drive."
            )
            if on_complete:
                on_complete()
            return
        ...
        if status != "ok":
            print(f"[Verificação] Erro inesperado: {result.get('error')}")
            if not silent_if_unavailable:
                messagebox.showerror(
                    "Verificação",
                    "Falha ao verificar. Tente novamente quando estiver em uma sessão."
        ...
        if self.show_scan_popup.get():
            messagebox.showinfo(
                "Verificação",
                f"{len(clean_vars)} controles 'dc' configurados para este carro."
            )
        if on_complete:
            on_complete()
    ...
    def notify_overlay_status(self, text: str, color: str):
        ...
        self.ui(
            self.root.after,
            2000,
            lambda: self.overlay.update_status_text("HUD pronto", "white")
        )
    ...
    def _update_overlay_feedback(self):
        ...
        if state["abs_active"] >= cfg["abs_hold_s"]:
            self._push_overlay_alert(
                "ABS ativo por muito tempo: alivie o freio ou reduza o ABS.",
                "orange",
                cfg,
                now
            )
        ...
        if state["tc_active"] >= cfg["tc_hold_s"]:
            self._push_overlay_alert(
                "TC acionando constantemente: considere reduzir o TC ou mudar o mapa.",
                "orange",
                cfg,
                now
            )
    ...
    def open_timing_window(self):
        ...
        GlobalTemporizaçãoWindow(self.root, self.save_timing_config)
    ...
    def save_config(self):
        ...
        try:
            ...
            print(f"[SALVAR] Erro ao salvar configuração: {e}")
    ...
    def _format_vosk_status(self) -> str:
        ...
        if engine != "vosk":
            if model_path:
                return f"Modelo Vosk selecionado: {model_name or model_path}"
            return "Usando o reconhecedor de fala do Windows"

        if not HAS_VOSK:
            if VOSK_IMPORT_ERROR:
                return f"Vosk indisponível: {VOSK_IMPORT_ERROR}"
            return "Vosk não instalado"

        if not model_path:
            return "Selecione uma pasta de modelo Vosk"

        if voice_listener._vosk_error:
            return f"Erro do modelo: {voice_listener._vosk_error}"

        if voice_listener.vosk_model is not None:
            name = model_name or model_path
            return f"Vosk pronto: {name}"
    ...
    def _format_whisper_status_details(self) -> str:
        ...
        if not self.whisper_binary_path.get():
            if self.whisper_model_path.get():
                ...
                return f"Modelo Whisper selecionado: {model_name}"
            return "Selecione o executável do whisper.cpp"
        ...
        if not self.whisper_model_path.get():
            ...
            return f"whisper.cpp selecionado: {exe_name} (escolha um modelo)"

        if not os.path.exists(self.whisper_model_path.get()):
            return "Modelo não encontrado"

        if voice_listener._whisper_error:
            return f"Erro do Whisper: {voice_listener._whisper_error}"

        model_name = os.path.basename(self.whisper_model_path.get())
        exe_name = os.path.basename(self.whisper_binary_path.get())
        return f"Whisper pronto: {exe_name} | {model_name}"
    ...
    def choose_vosk_model(self):
        ...
        path = filedialog.askdirectory(title="Selecionar pasta do modelo Vosk")
        if not path:
            return
    ...
    def choose_whisper_binary(self):
        ...
        path = filedialog.askopenfilename(
            title="Selecione o executável do whisper.cpp",
    ...
    def choose_whisper_model(self):
        ...
        path = filedialog.askopenfilename(
            title="Selecionar modelo Whisper (.bin/.gguf)",
    ...
    def open_voice_test_dialog(self):
        ...
        if not HAS_SPEECH:
            messagebox.showinfo(
                "Voz indisponível",
                "Instale o pacote 'speech_recognition' para habilitar o controle por voz."
            )
            return
    ...
    def restore_defaults(self):
        ...
        if not messagebox.askyesno(
            "Restaurar padrões",
            "Isso excluirá seu arquivo de configuração e reiniciará o aplicativo. Continuar?"
        ):
            return
        ...
        try:
            ...
            messagebox.showerror(
                "Erro",
                f"Falha ao excluir configuração: {exc}"
            )
            return

        messagebox.showinfo(
            "Padrões restaurados",
            "Configuração redefinida. O aplicativo será reiniciado agora."
        )
        restart_program()
