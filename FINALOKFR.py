...
class VoiceTestDialog(tk.Toplevel):
    ...
    def __init__(
        ...
        ) or "Aucune phrase configurée."

        tk.Label(
            self,
            text="Phrases disponibles :",
            font=("Arial", 10, "bold")
        ...
        self.status_var = tk.StringVar(value="En attente du test...")
        self.heard_var = tk.StringVar(value="(rien pour l'instant)")

        self.btn_listen = tk.Button(
            self,
            text="🎤 Écoutez et testez",
        ...
        tk.Label(manual, text="Exécutez la phrase manuellement :").pack(
            anchor="w"
        )
    ...
    def start_listen(self):
        ...
        self.btn_listen.config(state="disabled", text="Écoute...")
        self.status_var.set("Énoncez la commande configurée maintenant...")
        self.heard_var.set("(écoute)")
    ...
    def _listen_worker(self):
        ...
        def finalize():
            self.btn_listen.config(state="normal", text="🎤 Écoutez et testez")
            if error:
                self.status_var.set(f"Erreur lors de l'écoute : {error}")
                return

            if phrase is None:
                self.status_var.set("Voix indisponible.")
                return

            normalized = phrase.strip()
            self.heard_var.set(normalized or "(rien de reconnu)")

            if not normalized:
                self.status_var.set("Aucune phrase n'a été reconnue.")
                return

            triggered = self._trigger_phrase(normalized)
            if triggered:
                self.status_var.set("Macro déclenchée avec succès !")
            else:
                self.status_var.set("Phrase reconnue, mais aucune macro n'est liée.")
    ...
    def run_manual_phrase(self):
        ...
        if not phrase:
            self.status_var.set("Entrez une phrase à tester.")
            return

        if self._trigger_phrase(phrase):
            self.status_var.set("Macro exécutée manuellement.")
        else:
            self.status_var.set("Aucune macro n'est liée à cette phrase.")
...
class DeviceSelector(tk.Toplevel):
    ...
    def __init__(self, parent, current_allowed: List[str], callback: Callable[[List[str]], None]):
        ...
        tk.Label(
            self,
            text="Sélectionnez les appareils que l'application peut utiliser",
            font=("Arial", 10, "bold"),
            pady=10
        ).pack()
        ...
        tk.Label(
            self,
            text="Cochez/décochez pour autoriser/interdire l'utilisation de l'appareil",
            fg="gray"
        ).pack()
...
class ScrollableFrame(tk.Frame):
    ...
    def __init__(self, parent, *args, **kwargs):
        ...
        self.inner.bind(
            "<Configure>",
            lambda e: canvas.configure(scrollregion=canvas.bbox("tous"))
        )
...
class OverlayConfigTab(tk.Frame):
    ...
    def __init__(self, parent, app):
        ...
        tk.Label(
            self.body,
            text="HUD / Configuration de superposition",
            font=("Arial", 11, "bold")
        ).pack(anchor="w", pady=(5, 5))

        # Global appearance settings
        appearance_frame = tk.LabelFrame(self.body, text="Apparence globale du HUD")
        ...
        tk.Label(appearance_frame, text="Taille de la police :").grid(
        ...
        tk.Label(appearance_frame, text="Opacité :").grid(
        ...
        self.scale_opacity.set(self.app.overlay.style_cfg.get("opacité", 0.85))
        self.scale_opacity.grid(row=3, column=1, padx=5, pady=5, sticky="we")

        tk.Label(appearance_frame, text="Largeur de superposition :").grid(
        ...
        self.scale_width.set(self.app.overlay.style_cfg.get("largeur", 250))
        self.scale_width.grid(row=4, column=1, padx=5, pady=5, sticky="we")

        tk.Label(appearance_frame, text="Hauteur de superposition :").grid(
        ...
        self.scale_height.set(self.app.overlay.style_cfg.get("hauteur", 150))
        ...
        feedback_frame = tk.LabelFrame(
            self.body, text="Seuils de commentaires d'assistance (par voiture)"
        )
        feedback_frame.pack(fill="x", padx=5, pady=5)

        tk.Checkbutton(
            feedback_frame,
            text="Afficher les indices ABS / TC / glissement sur le HUD",
        ...
        variables_frame = tk.LabelFrame(
            self.body, 
            text="Variables à afficher (par voiture)"
        )
        ...
        tk.Label(header, text="Afficher", width=8, anchor="w").pack(
            side="left", padx=2
        )
        tk.Label(header, text="Nom interne", width=25, anchor="w").pack(
            side="left", padx=2
        )
        tk.Label(header, text="Étiquette du HUD", width=20, anchor="w").pack(
            side="left", padx=2
        )
        ...
        tk.Label(
            self.body,
            text="Les sélections variables et les étiquettes sont enregistrées par voiture."
    ...
    def pick_background_color(self):
        ...
        color = colorchooser.askcolor(title="Couleur d'arrière-plan")[1]
        if color:
    ...
    def _on_feedback_change(self, *_args):
        ...
        car = self.app.current_car or "Voiture générique"
...
class GenericController:
    ...
    def _run_target_loop(self):
        ...
        timing_profile = _normalize_timing_config(GLOBAL_TIMING).get("profil", "agressif")
...
class ControlTab(tk.Frame):
    ...
    def __init__(self, parent, controller: GenericController, label_name: str, app):
        ...
        tk.Button(
            keys_frame,
            text="Tester le temps minimal personnalisé",
        ...
        tk.Label(
            presets_frame,
            text="RESET renvoie toujours à une valeur de base (par exemple, 0 ou 50). ",
            fg="gray",
            font=("Arial", 8),
            wraplength=760,
            justify="left"
        ...
        tk.Label(
            header,
            text="Phrase de déclenchement vocal",
            width=18,
            anchor="w",
            font=("Arial", 8, "bold")
        ).pack(side="left", padx=5)
    ...
    def run_bot_timing_probe(self):
        ...
        def _worker():
            try:
                ...
                self.after(
                    0,
                    lambda msg=error_msg: messagebox.showerror("Clés manquantes", msg)
                )
                return
            ...
            if suggested is None:
                ...
                self.after(0, lambda: messagebox.showinfo("Résultat de la sonde", msg))
    ...
    def bind_game_key(self, direction: str):
        ...
        if self.app.app_state != "CONFIG":
            messagebox.showinfo("Avis", "Entrez d'abord en mode CONFIG.")
            return
        ...
        btn.config(text="APPUYEZ SUR LA TOUCHE...", bg="jaune")
    ...
    def _config_bind_button(self, button: tk.Button, data_store: Dict[str, Any]):
        ...
        def on_click():
            ...
            if code and code != "CANCEL":
                ...
                button.config(text="Définir la liaison", bg="#f0f0f0")
    ...
    def add_preset_row(
        ...
        if self.app.app_state != "CONFIG":
            value_entry.config(state="lecture seule")

        bind_button = tk.Button(frame, text="Définir la liaison", width=12)
        ...
        if self.app.app_state != "CONFIG":
            voice_entry.config(state="lecture seule")
    ...
    def monitor_loop(self):
        ...
        try:
            self.lbl_monitor.config(text=f"Actuel : {text}")
        except Exception:
            pass
    ...
    def set_config(self, config: Dict[str, Any]):
        ...
        self.btn_increase.config(text=config.get("key_increase_text", "Définir l'augmentation (+)"))
        self.btn_decrease.config(text=config.get("key_decrease_text", "Définir la diminution (-)"))
...
class ComboTab(tk.Frame):
    ...
    def __init__(
        ...
        tk.Label(
            body,
            text="⚡ Ajustements combinés (un déclencheur -> plusieurs variables)",
            fg="orange",
            font=("Arial", 10, "bold")
        ).pack(pady=5)
    ...
    def add_dynamic_row(
        ...
        bind_button = tk.Button(
            frame,
            text="RESET" if is_reset else "Définir la liaison",
            width=15,
            fg="red" if is_reset else "black"
        )
        ...
        for var_name in self.var_names:
            ...
            if self.app.app_state != "CONFIG":
                entry.config(state="lecture seule")
...
class GlobalTimingWindow(tk.Toplevel):
    ...
    def __init__(self, parent, callback_save: Callable):
        ...
        # Profile selection
        profiles_frame = tk.LabelFrame(timing_frame, text="Profils de comportement")
        ...
        tk.Radiobutton(
            profiles_frame,
            text="🤖 BOT (expérimental, délai proche de zéro)",
        ...
        tk.Radiobutton(
            profiles_frame,
            text="🤖 BOT Stable (rapide, plus fiable)",
            variable=self.var_profile,
            value="bot_safe",
            command=self._on_profile_change
        ).pack(anchor="w", padx=5, pady=2)
        ...
        tk.Radiobutton(
            profiles_frame,
            text="😈 Agressif (rapide, robotique)",
            variable=self.var_profile,
            value="aggressive",
            command=self._on_profile_change
        ).pack(anchor="w", padx=5, pady=2)
        ...
        tk.Radiobutton(
            profiles_frame,
            text="🛠 Personnalisé (définir les valeurs ci-dessous)",
            variable=self.var_profile,
            value="custom",
            command=self._on_profile_change
        ...
        self.custom_frame = tk.LabelFrame(
            timing_frame, 
            text="Paramètres personnalisés (ce profil uniquement)"
        )
        ...
        tk.Label(self.custom_frame, text="Intervalle Min (ms):").grid(
        ...
        tk.Label(self.custom_frame, text="Intervalle Max (ms):").grid(
        ...
        tk.Label(self.custom_frame, text="Plage (+/- ms):").grid(
...
class iRacingControlApp:
    ...
    def _create_menu(self):
        ...
        options_menu.add_command(
            label="Ajustements de synchronisation",
            command=self.open_timing_window
        )
        options_menu.add_command(
            label="Paramètres voix/audio",
            command=self.open_voice_audio_settings
        )
        options_menu.add_separator()
        options_menu.add_command(
            label="Afficher/Masquer la superposition",
        ...
        options_menu.add_separator()
        options_menu.add_command(
            label="Restaurer les paramètres par défaut (supprimer la configuration)",
    ...
    def _create_main_ui(self):
        ...
        presets_frame = tk.LabelFrame(
            steps_column,
            text="Étape 1 : Choisissez votre voiture et suivez"
        )
        ...
        tk.Label(selector_frame, text="Voiture :").pack(side="gauche")
        ...
        tk.Label(selector_frame, text="Piste :").pack(side="gauche")
        ...
        devices_frame = tk.LabelFrame(
            steps_column,
            text="Étape 2 : Confirmer les périphériques d'entrée (joystick/molette)"
        )
        devices_frame.pack(fill="x", pady=(0, 8))

        self.check_safe = tk.Checkbutton(
            devices_frame,
            text="Mode clavier uniquement (nécessite un redémarrage)",
        ...
        scan_frame = tk.LabelFrame(
            steps_column,
            text="Étape 3 : Analyser les commandes du pilote"
        )
        scan_frame.pack(fill="x")

        self.btn_scan = tk.Button(
            scan_frame,
            text="Analyser les commandes de la voiture sélectionnée",
        ...
        tk.Label(
            scan_frame,
            text="Astuce : Analyser après avoir modifié les appareils ou les préréglages pour maintenir les liaisons synchronisées.",
            fg="gray",
            font=("Arial", 9)
        ...
        options_notebook.add(general_tab, text="Paramètres généraux")
        ...
        general_left = tk.LabelFrame(
            general_container,
            text="Préréglages et appareils"
        )
        general_left.grid(row=0, column=0, sticky="nsew", pady=(0, 8))

        tk.Checkbutton(
            general_left,
            text="Enregistrement automatique des modifications prédéfinies (touches de raccourci/macros)",
        ...
        tk.Checkbutton(
            general_left,
            text="Verrouillage de la sélection de voiture/piste (géré automatiquement)",
        ...
        tk.Checkbutton(
            general_left,
            text="Détection automatique de voiture/piste via iRacing",
        ...
        tk.Checkbutton(
            general_left,
            text="Scan automatique lorsque la voiture/piste change",
        ...
        tk.Checkbutton(
            general_left,
            text="Afficher la fenêtre contextuelle de démarrage au démarrage",
        ...
        stability_frame = tk.LabelFrame(
            general_container,
            text="Automation et raccourcis"
        )
        stability_frame.grid(row=1, column=0, sticky="nsew")

        tk.Button(
            stability_frame,
            text="Ajustements de synchronisation",
            command=self.open_timing_window
        ).pack(fill="x", padx=8, pady=(8, 6))

        tk.Button(
            stability_frame,
            text="Options vocales/audio",
            command=self.open_voice_audio_settings
        ).pack(fill="x", padx=8, pady=(0, 6))

        tk.Checkbutton(
            stability_frame,
            text="Redémarrer avant de réanalyser les commandes (après la première analyse)",
        ...
        tk.Checkbutton(
            stability_frame,
            text="Redémarrer et analyser automatiquement lorsque vous rejoignez une session de course",
        ...
        tk.Checkbutton(
            stability_frame,
            text="Afficher la fenêtre contextuelle de fin d'analyse",
        ...
        tk.Checkbutton(
            stability_frame,
            text="Continuer d'essayer d'atteindre les cibles des raccourcis clavier (sans délai d'attente)",
        ...
        tk.Label(
            clear_frame,
            text="Effacer le raccourci clavier cible (facultatif) :"
        ).pack(side="left")
        self.btn_clear_target_bind = tk.Button(
            clear_frame,
            text="Définir le raccourci clavier Effacer",
            width=18,
        ...
        tk.Label(
            rescan_frame,
            text="Touche de raccourci de nouvelle analyse manuelle (redémarrage + analyse + chargement du préréglage) :"
        ).pack(side="left")
        self.btn_manual_rescan_bind = tk.Button(
            rescan_frame,
            text="Définir la touche de raccourci de nouvelle analyse",
            width=18,
    ...
    def _build_voice_audio_tab(self, parent: tk.Widget):
        ...
        if not HAS_SPEECH:
            tk.Label(
                toggles_frame,
                text="(Installer la « reconnaissance vocale » pour la voix)",
                fg="gray",
                font=("Arial", 8)
            ).pack(side="left", padx=4)

        engine_frame = tk.LabelFrame(parent, text="Moteur de reconnaissance")
        engine_frame.pack(fill="x", padx=2, pady=6)

        engine_row = tk.Frame(engine_frame)
        engine_row.pack(fill="x", padx=6, pady=2)

        ttk.Label(engine_row, text="Moteur vocal :").pack(side="gauche", padx=4)
        ...
        self.btn_vosk_model = tk.Button(
            model_row,
            text="Sélectionnez le modèle Vosk...",
        ...
        device_frame = tk.LabelFrame(parent, text="Périphériques d'entrée/sortie")
        device_frame.pack(fill="x", padx=2, pady=6)

        mic_row = tk.Frame(device_frame)
        mic_row.pack(fill="x", padx=6, pady=2)

        ttk.Label(mic_row, text="Microphone :").pack(side="gauche")
        ...
        ttk.Label(out_row, text="Sortie audio (TTS) :").pack(side="gauche")
        ...
        tuning_frame = tk.LabelFrame(
            parent,
            text="Réglage vocal (précision et vitesse)"
        )
        ...
        ttk.Label(tuning_row_1, text="Bruit(s) ambiant(s) :").pack(side="gauche")
        ttk.Spinbox(
            tuning_row_1,
            from_=0.0,
            to=3.0,
        ...
        ttk.Label(tuning_row_1, text="Durée(s) maximale(s) de phrase :").pack(side="gauche")
        ttk.Spinbox(
            tuning_row_1,
            from_=0.2,
            to=6.0,
        ...
        ttk.Label(tuning_row_2, text="Délai(s) d'expiration initial(s) :").pack(side="gauche")
        ttk.Spinbox(
            tuning_row_2,
            from_=0.0,
            to=5.0,
        ...
        ttk.Label(tuning_row_2, text="Délai(s) d'expiration continu(s) :").pack(side="gauche")
        ttk.Spinbox(
            tuning_row_2,
            from_=0.0,
            to=5.0,
        ...
        ttk.Label(tuning_row_2, text="Énergie minimale : ").pack(side="gauche")
        ttk.Entry(
            tuning_row_2,
            width=8,
    ...
    def toggle_mode(self):
        ...
        if self.app_state == "RUNNING":
            ...
            self.btn_mode.config(
                text="Mode : CONFIG (Cliquez pour enregistrer et exécuter)",
                bg="orange"
            )
    ...
    def _set_clear_target_bind(self):
        ...
        if self.btn_clear_target_bind:
            self.btn_clear_target_bind.config(text="...", bg="jaune")
    ...
    def _set_manual_rescan_bind(self):
        ...
        if self.btn_manual_rescan_bind:
            self.btn_manual_rescan_bind.config(text="...", bg="jaune")
        self.root.update_idletasks()
    ...
    def action_save_preset(self):
        ...
        if not car or not track:
            messagebox.showwarning("Erreur", "Définir la voiture et la piste.")
            return
    ...
    def _save_preset_for_pair(
        ...
        for var_name, tab in self.tabs.items():
            current_data["onglets"][var_name] = tab.get_config()
        ...
        self.update_preset_ui()
        if show_message:
            messagebox.showinfo("Enregistré", f"Préréglage enregistré pour {car} @ {track}")
    ...
    def rebuild_tabs(self, vars_list: List[Tuple[str, bool]]):
        ...
        self.notebook.add(overlay_frame, text="HUD / Superposition")

        # Load overlay for current car
        car = self.current_car or "Voiture générique"
    ...
    def choose_vosk_model(self):
        ...
        path = filedialog.askdirectory(title="Sélectionner le dossier du modèle Vosk")
        if not path:
            return
    ...
    def choose_whisper_binary(self):
        ...
        path = filedialog.askopenfilename(
            title="Sélectionner l'exécutable Whisper.cpp",
    ...
    def choose_whisper_model(self):
        ...
        path = filedialog.askopenfilename(
            title="Sélectionner le modèle Whisper (.bin/.gguf)",
